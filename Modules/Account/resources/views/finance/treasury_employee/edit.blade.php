...
@section('content')
    ...
    <div class="content-body">
        ...
        <div class="card">
            <div class="card-body">
                <form id="expenses_form" action="{{ route('treasury_employee.update',$treasury_employee->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="employee_id">
                                ...
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ old('employee_id',$treasury_employee->employee_id) == $employee->id ? 'selected' : '' }}>{{ $employee->full_name }}</option>
                                @endforeach
                            </select>
                            @error('employee_id')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" name="treasury_id">
                                ...
                                @foreach($treasuries as $treasury)
                                    <option value="{{ $treasury->id }}" {{ old('treasury_id',$treasury_employee->treasury_id) == $treasury->id ? 'selected' : '' }}>{{ $treasury->name }}</option>
                                @endforeach
                            </select>
                            @error('treasury_id')
                            <span class="text-danger" id="basic-default-name-error" class="error">
