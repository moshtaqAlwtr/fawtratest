                                ...
                                <select name="sub_account" class="form-control select2" id="sub_account">
                                    ...
                                    @foreach ($Accounts as $account)
                                        <option value="{{ $account->id }}">
                                            {{ $account->name }}-{{ $account->client->code??'' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
...
<script>
    ...
    window.routes = {
          incomes: {
            show: "{{ route('incomes.show', ':id') }}",
            print_thermal: "{{ route('incomes.print', [':id', 'thermal']) }}",
            print_normal: "{{ route('incomes.print', [':id', 'normal']) }}",
            cancel: "{{ route('incomes.cancel', ':id') }}"
        }
    };
</script>
...
<script>
    ...
    function loadData(page = 1) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                updateResults(response.incomes.data);
            ...
            error: function(xhr, status, error) {
                ...
                $('#loading-spinner').hide();
                $('#results-container').html('<div class="alert alert-danger">حدث خطأ في تحميل البيانات</div>').show();
            }
        });
    }
    ...
    function updateTotals(totals, accountSetting) {
        ...
        const totalsHtml = `
            <div class="text-center">
                ...
                <h2 class="text-white">${currencySymbol} ${totals.totalLast7Days}</h2>
            </div>
            <div class="text-center">
                <p class="text-muted">آخر 30 يوم</p>
                <h2 class="text-white">${currencySymbol} ${totals.totalLast30Days}</h2>
            </div>
            <div class="text-center">
                <p class="text-muted">آخر 365 يوم</p>
                <h2 class="text-white">${currencySymbol} ${totals.totalLast365Days}</h2>
            </div>
        `;
    ...
    function updateResults(incomes) {
        if (incomes.length === 0) {
            $('#results-container').html(`
                <div class="alert alert-danger text-xl-center" role="alert">
                    <p class="mb-0">لا توجد سندات قبض</p>
                </div>
            `);
            return;
        }
            ...
            html += `
                <tr>
                    <td style="width: 80%">
                        ...
                        ${income.attachments ? `<img src="${window.baseUrl}/assets/uploads/incomes/${income.attachments}" alt="img" width="100"><br>` : ''}
                        ...
                        <div class="btn-group">
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item" href="${window.routes.incomes.show.replace(':id', income.id)}">
                                            <i class="fa fa-eye me-2 text-primary"></i>عرض
                                        </a>
                                    </li>
                                    <li>
                                        <button type="button" class="dropdown-item text-danger"
                                            onclick="confirmCancel(${income.id})">
                                            <i class="fa fa-times me-2"></i>إلغاء
                                        </button>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="${window.routes.incomes.print_thermal.replace(':id', income.id)}">
                                            <i class="fa fa-print me-2 text-info"></i>طباعة سند قبض حراري
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="${window.routes.incomes.print_normal.replace(':id', income.id)}">
    ...
    function buildRoute(routeName, params = {}) {
        let route = window.routes[routeName];
    ...
    function confirmCancel(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const cancelUrl = window.routes.incomes.cancel.replace(':id', id);
