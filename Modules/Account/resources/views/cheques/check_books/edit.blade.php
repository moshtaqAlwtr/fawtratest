...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">تعديل دفتر الشيكات</h2>
    ...
    <div class="content-body">
        <div class="container-fluid">
            <form class="form-horizontal" action="{{ route('check_books.update', $checkbook->id) }}" method="POST">
                @csrf
                <div class="card">
                    <div class="card-body">
                ...
                <div class="card">
                    <div class="card-content">
                        <div class="card-body">
                            <h4 class="card-title">معلومات دفتر الشيكات</h4>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="form-body row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control select2" id="basicSelect" name="bank_id">
                                        ...
                                        @foreach ($bank_accounts as $bank_account)
                                            <option value="{{ $bank_account->id }}" {{ $checkbook->bank_id == $bank_account->id ? 'selected' : '' }}>{{ $bank_account->bank_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" placeholder="رقم دفتر الشيكات" name="cheque_book_number" value="{{ $checkbook->cheque_book_number }}">
                                </div>

                                <div class="form-group col-md-3">
                                    <label for="">الرقم المسلسل الاول <span style="color: red">*</span></label>
                                    <input type="number" id="start_serial_number" class="form-control" name="start_serial_number" value="{{ $checkbook->start_serial_number }}">
                                ...
                                <div class="form-group col-md-3">
                                    ...
                                    <input type="number" id="end_serial_number" class="form-control" name="end_serial_number" value="{{ $checkbook->end_serial_number }}">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control select2" id="basicSelect" name="currency" required>
                                        <option value="USD" {{ $checkbook->currency == 'USD' ? 'selected' : '' }}>الدولار الأمريكي (USD)</option>
                                        <option value="EUR" {{ $checkbook->currency == 'EUR' ? 'selected' : '' }}>اليورو (EUR)</option>
                                        <option value="JPY" {{ $checkbook->currency == 'JPY' ? 'selected' : '' }}>الين الياباني (JPY)</option>
                                        <option value="GBP" {{ $checkbook->currency == 'GBP' ? 'selected' : '' }}>الجنيه الإسترليني (GBP)</option>
                                        <option value="AUD" {{ $checkbook->currency == 'AUD' ? 'selected' : '' }}>الدولار الأسترالي (AUD)</option>
                                        <option value="CAD" {{ $checkbook->currency == 'CAD' ? 'selected' : '' }}>الدولار الكندي (CAD)</option>
                                        <option value="CHF" {{ $checkbook->currency == 'CHF' ? 'selected' : '' }}>الفرنك السويسري (CHF)</option>
                                        <option value="CNY" {{ $checkbook->currency == 'CNY' ? 'selected' : '' }}>اليوان الصيني (CNY)</option>
                                        <option value="SAR" {{ $checkbook->currency == 'SAR' ? 'selected' : '' }}>الريال السعودي (SAR)</option>
                                        <option value="AED" {{ $checkbook->currency == 'AED' ? 'selected' : '' }}>الدرهم الإماراتي (AED)</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control" name="status" required>
                                        <option value="0" {{ $checkbook->status == 0 ? 'selected' : '' }}>نشط</option>
                                        <option value="1" {{ $checkbook->status == 1 ? 'selected' : '' }}>غير نشط</option>
                                    </select>
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="feedback2" class="sr-only">الملاحظات </label>
                                    <textarea id="feedback2" class="form-control" rows="2" placeholder="الملاحظات" name="notes">{{ $checkbook->notes }}</textarea>
                                </div>
