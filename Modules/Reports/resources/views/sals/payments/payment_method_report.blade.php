                ...
                <div class="col-md-8">
                    ...
                    <nav class="breadcrumb-custom">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active">تقرير المدفوعات حسب طريقة الدفع</li>
                        </ol>
                    </nav>
                </div>
                ...
                <form id="reportForm">
                    <div class="row g-4">
                        ...
                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern"><i class="fas fa-credit-card me-2"></i>طريقة الدفع</label>
                        ...
                        <div class="col-lg-3 col-md-6">
                            ...
                            <select name="report_type" id="report_type" class="form-control select2">
                                <option value="both">المدفوعات وسندات القبض</option>
                                <option value="payments">المدفوعات فقط</option>
                                <option value="receipts">سندات القبض فقط</option>
                            </select>
                        </div>
        ...
        <div class="row mb-4 fade-in" id="totalsSection">
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card primary">
                    ...
                    <div class="stats-value" id="totalReceipts">{{ number_format($totals['total_receipts'] ?? 0, 2) }}
                    </div>
        ...
        <div class="card-modern fade-in mb-4" id="chartSection">
            <div class="card-header-modern">
                <h5 class="mb-0">
                    ...
                    الرسم البياني للمدفوعات حسب طريقة الدفع
                </h5>
            </div>
            <div class="card-body-modern">
        ...
        <div class="card-modern fade-in" id="reportContainer" style="position: relative;">
            ...
            <div class="card-header-modern">
                <h5 class="mb-0" id="reportTitle">
                    ...
                    تقرير المدفوعات حسب طريقة الدفع من {{ $fromDate->format('d/m/Y') }} إلى {{ $toDate->format('d/m/Y') }}
                </h5>
            </div>
                    ...
                    <table class="table table-modern mb-0">
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-credit-card me-2"></i>طريقة الدفع</th>
                                ...
                                <th><i class="fas fa-file-alt me-2"></i>النوع</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('salesReports.paymentMethodReportAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير المدفوعات حسب طريقة الدفع من ${data.from_date} إلى ${data.to_date}
                `);
            ...
            function updateChart(chartData) {
                ...
                paymentMethodChart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            label: 'المبلغ (ريال)',
                            data: chartData.payments.map((payment, index) => payment + chartData.receipts[index]),
                            backgroundColor: [
                                'rgba(16, 185, 129, 0.8)',
                                'rgba(59, 130, 246, 0.8)',
                                'rgba(245, 158, 11, 0.8)',
            ...
            function updateTableBody(groupedData, totals) {
                ...
                // تكرار عبر المجموعات
                Object.keys(groupedData).forEach(methodId => {
                    const methodData = groupedData[methodId];
                    const paymentMethod = methodData.payment_method;
                    ...
                    tableHtml += `
                        <tr class="table-employee-header">
                            <td colspan="9">
                                ...
                                <span class="ms-3">
                                    (المدفوعات: ${formatNumber(methodData.total_payments)} ريال |
                                    سندات القبض: ${formatNumber(methodData.total_receipts)} ريال |
                                    الإجمالي: ${formatNumber(methodData.total_amount)} ريال)
                                </span>
                            </td>
                        </tr>
                    `;
                    ...
                    // عرض المدفوعات
                    if (methodData.payments && methodData.payments.length > 0) {
                        methodData.payments.forEach(payment => {
                            tableHtml += `<tr class="payment-row">`;
                            tableHtml += `<td>${counter++}</td>`;
                            tableHtml += `<td>${paymentMethod.name}</td>`;
                            tableHtml += `<td>${formatDate(payment.payment_date)}</td>`;
                            tableHtml += `<td>${payment.invoice && payment.invoice.client ? payment.invoice.client.trade_name : 'غير محدد'}</td>`;
                            tableHtml += `<td>${payment.employee ? payment.employee.name : 'غير محدد'}</td>`;
                            tableHtml += `<td><span class="badge bg-success"><i class="fas fa-money-bill-wave me-1"></i>مدفوعة</span></td>`;
                    ...
                    // عرض سندات القبض
                    if (methodData.receipts && methodData.receipts.length > 0) {
                        methodData.receipts.forEach(receipt => {
                            tableHtml += `<tr class="receipt-row">`;
                            tableHtml += `<td>${counter++}</td>`;
                            tableHtml += `<td>${paymentMethod.name}</td>`;
                            tableHtml += `<td>${formatDate(receipt.date)}</td>`;
                            tableHtml += `<td>${receipt.client ? receipt.client.trade_name : 'غير محدد'}</td>`;
                            tableHtml += `<td>${receipt.employee ? receipt.employee.name : 'غير محدد'}</td>`;
                            tableHtml += `<td><span class="badge bg-primary"><i class="fas fa-receipt me-1"></i>سند قبض</span></td>`;
                            tableHtml += `<td>${receipt.code || '--'}</td>`;
                    ...
                    // صف إجمالي طريقة الدفع
                    tableHtml += `
                        <tr class="table-employee-total">
                            <td colspan="7">
                                <i class="fas fa-calculator me-2"></i>
                                <strong>مجموع ${paymentMethod.name}</strong>
                            </td>
                            <td class="fw-bold">${formatNumber(methodData.total_amount)}</td>
                            <td>-</td>
                        </tr>
                    `;

                    grandPaymentsTotal += methodData.total_payments;
                    grandReceiptsTotal += methodData.total_receipts;
                    grandAmountTotal += methodData.total_amount;
                });
            ...
            function exportToExcel() {
                ...
                const fileName = `تقرير_المدفوعات_حسب_طريقة_الدفع_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
