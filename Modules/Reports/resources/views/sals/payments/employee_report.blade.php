                ...
                <div class="col-md-8">
                    ...
                    <nav class="breadcrumb-custom">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active">تقرير المدفوعات وسندات القبض</li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="card-modern fade-in mb-4" id="chartSection">
            <div class="card-header-modern">
                <h5 class="mb-0">
                    ...
                    الرسم البياني للمدفوعات وسندات القبض
                </h5>
            </div>
            <div class="card-body-modern">
                <div class="chart-container" style="position: relative; height: 400px;">
                    <canvas id="paymentsReceiptsChart"></canvas>
                </div>
            </div>
        </div>
        ...
        <div class="card-modern fade-in" id="reportContainer" style="position: relative;">
            ...
            <div class="card-header-modern">
                <h5 class="mb-0" id="reportTitle">
                    ...
                    تقرير المدفوعات وسندات القبض من {{ $fromDate->format('d/m/Y') }} إلى {{ $toDate->format('d/m/Y') }}
                </h5>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('salesReports.employeePaymentsReceiptsReportAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير المدفوعات وسندات القبض من ${data.from_date} إلى ${data.to_date}
                `);
            ...
            function updateChart(chartData) {
                const ctx = document.getElementById('paymentsReceiptsChart').getContext('2d');

                if (paymentsReceiptsChart) {
                    paymentsReceiptsChart.destroy();
                }

                paymentsReceiptsChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: chartData.labels,
                        datasets: [{
                                label: 'المدفوعات (ريال)',
                ...
                Object.keys(groupedData).forEach(employeeId => {
                    ...
                    tableHtml += `
                        <tr class="table-employee-header">
                            <td colspan="9">
                                ...
                                <span class="ms-3">
                                    (المدفوعات: ${formatNumber(employeeData.total_payments)} ريال |
                                    سندات القبض: ${formatNumber(employeeData.total_receipts)} ريال |
                    ...
                    if (employeeData.payments && employeeData.payments.length > 0) {
                        employeeData.payments.forEach(payment => {
                            ...
                            tableHtml +=
                                `<td>${payment.invoice && payment.invoice.client ? payment.invoice.client.trade_name : 'غير محدد'}</td>`;
                            tableHtml +=
                                `<td><span class="badge bg-success"><i class="fas fa-money-bill-wave me-1"></i>مدفوعة</span></td>`;
                            tableHtml += `<td>${payment.reference_number || '--'}</td>`;
                            tableHtml += `<td>${getPaymentMethodName(payment.payment_method)}</td>`;
                            tableHtml +=
                                `<td class="text-success fw-bold">${formatNumber(payment.amount)}</td>`;
                    ...
                    // عرض سندات القبض
                    if (employeeData.receipts && employeeData.receipts.length > 0) {
                        employeeData.receipts.forEach(receipt => {
                            tableHtml += `<tr class="receipt-row">`;
                            tableHtml += `<td>${counter++}</td>`;
                            tableHtml += `<td>${employee.name}</td>`;
                            tableHtml += `<td>${formatDate(receipt.date)}</td>`;
                            tableHtml +=
                                `<td>${receipt.account && receipt.account.client ? receipt.account.client.trade_name : 'غير محدد'}</td>`;
                            tableHtml +=
                                `<td><span class="badge bg-primary"><i class="fas fa-receipt me-1"></i>سند قبض</span></td>`;
                            tableHtml += `<td>${receipt.code || '--'}</td>`;
                            tableHtml +=
                                `<td>${receipt.treasury ? receipt.treasury.name : (receipt.account ? receipt.account.name : '--')}</td>`;
                            tableHtml +=
                                `<td class="text-primary fw-bold">${formatNumber(receipt.amount)}</td>`;
                    ...
                    grandReceiptsTotal += employeeData.total_receipts;
            ...
            // Date range function removed - no longer needed
            ...
            function exportToExcel() {
                ...
                const fileName =
                    `تقرير_المدفوعات_وسندات_القبض_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
        ...
        // Form validation function removed - no longer needed

        // Performance optimization functions removed - no longer needed

        // Date input handlers removed - no longer needed
    </script>
@endsection