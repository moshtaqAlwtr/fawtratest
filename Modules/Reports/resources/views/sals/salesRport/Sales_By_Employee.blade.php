        ...
        <div class="row mb-4 fade-in" id="totalsSection">
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card warning">
                    ...
                    <div class="stats-value" id="unpaidAmount">{{ number_format($totals['unpaid_amount'] ?? 0, 2) }}
                    </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card danger">
                    ...
                    <div class="stats-value" id="returnedAmount">{{ number_format($totals['total_returns'] ?? 0, 2) }}
                    </div>
...
$('.select2').select2({
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('salesReports.byEmployeeAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير المبيعات من ${data.from_date} إلى ${data.to_date}
                `);
            ...
            function updateTableBody(groupedInvoices, totals) {
                ...
                Object.keys(groupedInvoices).forEach(employeeId => {
                    ...
                    const employeeName = invoices[0].created_by_user ? invoices[0].created_by_user.name :
                        'موظف ' + employeeId;
                    ...
                    invoices.forEach(invoice => {
                        ...
                        tableHtml += `<td>${formatDate(invoice.invoice_date)}</td>`;
                        tableHtml +=
                            `<td>${invoice.client ? invoice.client.trade_name : 'غير محدد'}</td>`;

                        if (isReturn) {
                            tableHtml += `<td>-</td>`;
                            tableHtml += `<td>-</td>`;
                            tableHtml +=
                                `<td class="text-danger fw-bold">${formatNumber(invoice.grand_total)}</td>`;
                            tableHtml +=
                                `<td class="text-danger fw-bold">-${formatNumber(invoice.grand_total)}</td>`;
                            ...
                            const paidAmount = invoice.payment_status == 1 ? invoice.grand_total :
                                invoice.paid_amount;
                            const unpaidAmount = invoice.payment_status == 1 ? 0 : invoice
                                .due_value;

                            tableHtml +=
                                `<td class="text-success fw-bold">${formatNumber(paidAmount)}</td>`;
                            tableHtml +=
                                `<td class="text-warning fw-bold">${formatNumber(unpaidAmount)}</td>`;
                            tableHtml += `<td>-</td>`;
                            tableHtml +=
                                `<td class="fw-bold">${formatNumber(invoice.grand_total)}</td>`;
            ...
            function exportToExcel() {
                ...
                const today = new Date();
                const fileName =
                    `تقرير_المبيعات_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
