...
@section('css')
    ...
    <style>
        ...
        /* تحسين خانة البحث */
        #searchCostCenterTree {
            border-radius: 8px;
            border: 2px solid #e9ecef;
            transition: all 0.3s ease;
        }

        #searchCostCenterTree:focus {
            border-color: #28a745;
                ...
                <div class="col-md-8">
                    ...
                    <nav class="breadcrumb-custom">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active">تقرير مراكز التكلفة</li>
                        </ol>
                    </nav>
                </div>
            ...
            <div class="card-body-modern">
                <form id="reportForm">
                    <div class="row g-4">
                        ...
                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern"><i class="fas fa-eye me-2"></i>خيار العرض</label>
                            <select name="display_option" id="display_option" class="form-control select2">
                                <option value="">عرض جميع المراكز</option>
                                <option value="1">المراكز التي عليها معاملات</option>
                                <option value="2">إخفاء المراكز الصفرية</option>
                            </select>
                        </div>

                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern"><i class="fas fa-calendar-check me-2"></i>السنة المالية</label>
        ...
        <div class="row mb-4 fade-in" id="totalsSection">
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card warning">
                    ...
                    <div class="stats-label">إجمالي الرصيد (ريال)</div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card info">
                    ...
                    <div class="stats-value" id="costCentersCount">0</div>
                    <div class="stats-label">عدد مراكز التكلفة</div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card danger">
                    ...
                    <div class="stats-value" id="transactionsCount">0</div>
        ...
        <div class="card fade-in" id="reportContainer" style="position: relative;">
            ...
            <div class="card-body-modern p-0">
                <div class="table-responsive">
                    <table class="table table-modern mb-0 cost-center-tree">
                        <thead>
                            <tr>
                                <th><i class="fas fa-bullseye me-2"></i>مركز التكلفة</th>
                                <th><i class="fas fa-barcode me-2"></i>الكود</th>
                                <th class="text-danger">مدين</th>
                                <th class="text-success">دائن</th>
                                <th class="text-info">الرصيد</th>
                                <th class="text-secondary">عدد المعاملات</th>
                            </tr>
                        </thead>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('GeneralAccountReports.CostCentersReportAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                animateValue('#totalBalance', 0, Math.abs(data.totals.total_balance), 1000);
                animateValue('#costCentersCount', 0, data.totals.cost_centers_count, 1000);
                animateValue('#transactionsCount', 0, data.totals.transactions_count, 1000);

                // تحديث عنوان التقرير
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير مراكز التكلفة من ${data.from_date} إلى ${data.to_date}
                `);
                ...
                // تحديث جدول البيانات
                updateTableBody(data.cost_center_tree, data.totals);
            }
                ...
                costCenterChart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            label: 'الرصيد',
                            data: chartData.balance_amounts,
            ...
            // دالة عرض صف مركز التكلفة
            function renderCostCenterRow(costCenter, level, parentId = '') {
                let html = '';
                const paddingRight = level * 20;
                const levelClass = `cost-center-level-${level}`;
                const uniqueId = parentId ? `${parentId}-${costCenter.id}` : costCenter.id;
                ...
                const getBalanceClass = (balance) => {
                    if (balance == 0) return 'zero-amount';
                    return balance > 0 ? 'balance-positive' : 'balance-negative';
                };

                html += `
                    <tr class="cost-center-row ${levelClass}" data-level="${level}" data-cost-center-id="${costCenter.id}">
                        <td style="padding-right: ${paddingRight}px">
                            ...
                            <span class="cost-center-name">${costCenter.name}</span>
                            ${costCenter.is_main == 1 ? '<span class="badge bg-success ms-2">رئيسي</span>' : '<span class="badge bg-secondary ms-2">فرعي</span>'}
                        </td>
                        <td class="fw-bold cost-center-code">${costCenter.code}</td>
                        <td class="number-cell ${getAmountClass(costCenter.total_debit)}">${formatNumber(costCenter.total_debit)}</td>
                        <td class="number-cell ${getAmountClass(costCenter.total_credit)}">${formatNumber(costCenter.total_credit)}</td>
                        <td class="number-cell ${getBalanceClass(costCenter.total_balance)}">${formatNumber(costCenter.total_balance)}</td>
                        <td class="number-cell text-info">${formatNumber(costCenter.transactions_count)}</td>
                    </tr>
                `;

                // إضافة مراكز التكلفة الفرعية
                if (costCenter.children && costCenter.children.length > 0) {
                    html += `
                        <tr class="child-container" data-parent-id="${costCenter.id}">
                    ...
                    costCenter.children.forEach(child => {
                        html += renderCostCenterRow(child, level + 1, uniqueId);
                    });
            ...
            function updateTableBody(costCenterTree, totals) {
                let tableHtml = '';

                // عرض شجرة مراكز التكلفة
                costCenterTree.forEach(costCenter => {
                    tableHtml += renderCostCenterRow(costCenter, 0);
                });
                ...
                tableHtml += `
                    <tr class="table-grand-total">
                        ...
                        <td class="number-cell fw-bold">${formatNumber(totals.total_balance)}</td>
                        <td class="number-cell fw-bold">${formatNumber(totals.transactions_count)}</td>
                    </tr>
                `;
            ...
            function addTreeControlButtons() {
                const controlButtonsHtml = `
                    <div class="tree-controls mb-3" style="display: none;">
                        <div class="d-flex gap-2 align-items-center flex-wrap">
                            <button type="button" class="btn btn-sm btn-outline-success" id="expandAllBtn">
                            ...
                            <div class="input-group" style="width: 300px;">
                                ...
                                <button class="btn btn-outline-secondary btn-sm" type="button" id="clearSearch"></button>
                ...
                `;
                $('.table-responsive').prepend(controlButtonsHtml);
            }
        });
    </script>
@endsection
