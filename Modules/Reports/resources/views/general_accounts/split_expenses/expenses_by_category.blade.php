...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0"> تقرير المصروفات حسب التصنيف
                    </h2>
    ...
    <div class="container mt-5">
        ...
        <div class="card p-4 mb-3">
            <form action="{{ route('GeneralAccountReports.splitExpensesByCategory') }}" method="GET">
                        ...
                        <select class="form-control" id="inputGroupBy" name="group_by">
                            @foreach ($expensesCategory as $expense)
                                <option value="{{ $expense->id }}"
                                    {{ request('group_by') == $expense->id ? 'selected' : '' }}>{{ $expense->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div class="card p-4 mb-3">
            ...
            <div class="card p-4 mb-3">
                <h5>مخطط الإجمالي مع الضريبة حسب التصنيف</h5>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        @foreach ($groupedExpenses as $categoryId => $expensesInCategory)
                            ...
                            @foreach ($expensesInCategory as $expense)
                                <tr>
                                    <td>{{ $expense->code }}</td>
                                    <td>{{ $expense->date }}</td>
                            ...
                            <tr style="background-color: #e9ecef;">
                                ...
                                <td><strong>{{ number_format($expensesInCategory->sum('amount'), 2) }}</strong></td>
                                <td><strong>{{ number_format($expensesInCategory->sum('tax1_amount') + $expensesInCategory->sum('tax2_amount'), 2) }}</strong></td>
                                <td><strong>{{ number_format($expensesInCategory->sum('amount') + $expensesInCategory->sum('tax1_amount') + $expensesInCategory->sum('tax2_amount'), 2) }}</strong></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
