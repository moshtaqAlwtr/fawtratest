...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0"> تقرير المصروفات حسب العميل
                    </h2>
    ...
    <div class="container mt-5">
        ...
        <div class="card p-4 mb-3">
            <form action="{{ route('GeneralAccountReports.splitExpensesByClient') }}" method="GET">
                <div class="row g-3 align-items-center">
                    ...
                    <div class="col-md-3">
                        <label for="inputTaxType" class="form-label">العميل :</label>
                        <select class="form-control" id="inputTaxType" name="client">
        ...
        <div class="card p-4 mb-3">
            ...
            <div class="card p-4 mb-3">
                <h5>مخطط الإجمالي مع الضريبة حسب العميل</h5>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        @foreach ($groupedExpenses as $clientId => $expensesInClient)
                            ...
                            @foreach ($expensesInClient as $expense)
                                <tr>
                                    <td>{{ $expense->code }}</td>
                                    <td>{{ $expense->date }}</td>
                            ...
                            <tr style="background-color: #e9ecef;">
                                ...
                                <td><strong>{{ number_format($expensesInClient->sum('amount'), 2) }}</strong></td>
                                <td><strong>{{ number_format($expensesInClient->sum('tax1_amount') + $expensesInClient->sum('tax2_amount'), 2) }}</strong></td>
                                <td><strong>{{ number_format($expensesInClient->sum('amount') + $expensesInClient->sum('tax1_amount') + $expensesInClient->sum('tax2_amount'), 2) }}</strong></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        @foreach ($expenses as $expense)
                            <tr>
                                <td>{{ $expense->client->name ?? 'غير معروف' }}</td>
                                <td>{{ number_format($expense->amount, 2) }}</td>
                                <td>{{ number_format($expense->tax1_amount + $expense->tax2_amount, 2) }}</td>
                                <td>{{ number_format($expense->amount + $expense->tax1_amount + $expense->tax2_amount, 2) }}</td>
                            </tr>
