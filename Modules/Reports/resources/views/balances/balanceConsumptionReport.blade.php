                ...
                <div class="col-md-8">
                    ...
                    <nav class="breadcrumb-custom">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active">تقرير استهلاك الأرصدة</li>
                        </ol>
                    </nav>
                </div>
            ...
            <div class="card-body-modern">
                <form id="reportForm">
                    <div class="row g-4">
                        ...
                        <!-- Second Row -->
                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern"><i class="fas fa-file-invoice me-2"></i>رقم الفاتورة</label>
                            <input type="text" name="invoice_id" id="invoice_id" class="form-control" placeholder="رقم الفاتورة">
                        </div>
        ...
        <div class="row mb-4 fade-in" id="totalsSection">
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card danger">
                    ...
                    <div class="stats-value" id="totalConsumption">0</div>
                    <div class="stats-label">إجمالي الاستهلاك (ريال)</div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card warning">
                    ...
                    <div class="stats-value" id="averageConsumption">0</div>
                    <div class="stats-label">متوسط الاستهلاك (ريال)</div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card success">
                    ...
                    <div class="stats-value" id="activeConsumptions">0</div>
                    <div class="stats-label">العمليات النشطة</div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card info">
                    ...
                    <div class="stats-label">عدد عمليات الاستهلاك</div>
                </div>
            </div>
        </div>
        ...
        <div class="card-modern fade-in mb-4" id="chartSection">
            <div class="card-header-modern">
                <h5 class="mb-0">
                    ...
                    الرسم البياني لاستهلاك الأرصدة
                </h5>
            </div>
            <div class="card-body-modern">
                    ...
                    <table class="table table-modern mb-0">
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-calendar-check me-2"></i>تاريخ الاستهلاك</th>
                                <th><i class="fas fa-money-bill me-2"></i>المبلغ المستهلك (ريال)</th>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // تحديث البيانات عند تغيير التواريخ أو رقم الفاتورة
            $('#date_from, #date_to, #invoice_id').on('change', function() {
                if ($('#date_type').val() === 'custom' || $(this).attr('id') === 'invoice_id') {
                    loadReportData();
                }
            });
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('ClientReport.balanceConsumptionReportAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                animateValue('#totalConsumption', 0, data.totals.total_consumption, 1000);
                animateValue('#averageConsumption', 0, data.totals.average_consumption, 1000);
                animateValue('#activeConsumptions', 0, data.totals.active_consumptions, 1000);
                animateValue('#totalCount', 0, data.totals.total_count, 1000);

                // تحديث عنوان التقرير
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير استهلاك الأرصدة من ${data.from_date} إلى ${data.to_date}
                `);
                ...
                // تحديث جدول البيانات
                updateTableBody(data.consumptions);
            }
            ...
            function updateTableBody(consumptions) {
                let tableHtml = '';
                    ...
                    tableHtml = `
                        <tr>
                            <td colspan="9" class="text-center py-5">
                                ...
                                <p class="text-muted">لا توجد عمليات استهلاك أرصدة تطابق المعايير المحددة</p>
                            </td>
                        </tr>
                    `;
                } else {
                    consumptions.forEach((consumption, index) => {
                        const statusBadge = consumption.status == 1
                            ...
                            <tr class="consumption-row">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <div class="fw-semibold">${consumption.client ? consumption.client.trade_name : 'غير محدد'}</div>
                                            <small class="text-muted">${consumption.client ? consumption.client.code : '--'}</small>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge bg-info-subtle text-info">
                                        ...
                                        ${consumption.balance_type ? consumption.balance_type.name : 'غير محدد'}
                                    </span>
                                </td>
                                <td>
                                    ${consumption.invoice ?
                                        `<span class="badge bg-warning-subtle text-warning">
                                            ...
                                            ${consumption.invoice.invoice_number || consumption.invoice_id}
                                        </span>` :
                                        '<span class="text-muted">--</span>'
                                    }
                                </td>
                                <td>${consumption.consumption_date ? formatDate(consumption.consumption_date) : '--'}</td>
                                <td class="fw-bold text-danger">${formatNumber(consumption.used_balance)}</td>
                                <td>
                                    <span class="badge bg-secondary-subtle text-secondary">
                                        <i class="fas fa-building me-1"></i>
                                        ${consumption.client && consumption.client.branch ? consumption.client.branch.name : 'غير محدد'}
                                    </span>
                                </td>
                                <td>${statusBadge}</td>
                                <td>
                                    <span class="text-muted" title="${consumption.description || '--'}">
                                        ${consumption.description ? (consumption.description.length > 30 ? consumption.description.substring(0, 30) + '...' : consumption.description) : '--'}
                                    </span>
                                </td>
                            </tr>
                        `;
            ...
            function exportToExcel() {
                ...
                const today = new Date();
                const fileName = `تقرير_استهلاك_الأرصدة_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
