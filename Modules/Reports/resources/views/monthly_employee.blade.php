...
<body>
    ...
    <div class="section">
        <div class="section-title">
            <span>جميع الأنشطة خلال الشهر (مرتبة حسب الوقت)</span>
            <span class="section-count">{{ $allActivities->count() }}</span>
        </div>
        @if ($allActivities->count() > 0)
            <div class="date-range">الفترة من {{ $startDate->format('Y-m-d') }} إلى {{ $endDate->format('Y-m-d') }}</div>
            <table>
                <thead>
                    <tr>
        ...
        @else
            <div class="no-data">لا يوجد أنشطة مسجلة خلال هذا الشهر</div>
        @endif
    </div>
