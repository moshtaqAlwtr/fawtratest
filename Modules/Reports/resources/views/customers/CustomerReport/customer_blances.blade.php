            ...
            <div class="card-body-modern">
                <form id="reportForm">
                    <div class="row g-4">
                        ...
                        <!-- Second Row -->
                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern"><i class="fas fa-sitemap me-2"></i>تجميع حسب</label>
                        ...
                        <div class="col-lg-3 col-md-6 d-flex align-items-center">
                            <div class="form-check">
                                <input type="checkbox" id="show-details" name="show_details" class="form-check-input">
                                <label for="show-details" class="form-check-label">
                                    <i class="fas fa-list-ul me-2"></i>مشاهدة التفاصيل
                                </label>
                            </div>
                        </div>
        ...
        <div class="row mb-4 fade-in" id="totalsSection">
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card warning">
                    ...
                    <div class="stats-value" id="totalPayments">0</div>
            ...
            <div class="col-lg-3 col-md-6 mb-3">
                <div class="stats-card info">
                    ...
                    <div class="stats-value" id="totalBalance">0</div>
                    ...
                    <table class="table table-modern mb-0">
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-id-card me-2"></i>رقم الحساب</th>
                                ...
                                <th><i class="fas fa-history me-2"></i>الرصيد السابق</th>
                                <th><i class="fas fa-shopping-cart me-2"></i>إجمالي المبيعات</th>
                                <th><i class="fas fa-undo me-2"></i>إجمالي المرتجع</th>
                                <th><i class="fas fa-calculator me-2"></i>صافي المبيعات</th>
                                <th><i class="fas fa-money-bill-wave me-2"></i>إجمالي المدفوعات</th>
                                <th><i class="fas fa-clock me-2"></i>المبلغ المستحق</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('ClientReport.customerBalancesAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير أرصدة العملاء - تجميع حسب ${data.group_by || 'العميل'}
                `);
                ...
                // تحديث جدول البيانات
                updateTableBody(data.client_balances, data.totals);
            }
            ...
            function updateTableBody(clientBalances, totals) {
                let tableHtml = '';
                ...
                if (clientBalances && clientBalances.length > 0) {
                    ...
                    clientBalances.forEach((client, index) => {
                        // تحديث المجاميع
                        grandTotals.balance_before += parseFloat(client.balance_before || 0);
                        grandTotals.total_sales += parseFloat(client.total_sales || 0);
                        grandTotals.total_returns += parseFloat(client.total_returns || 0);
                        grandTotals.net_sales += parseFloat(client.net_sales || 0);
                        grandTotals.total_payments += parseFloat(client.total_payments || 0);
                        grandTotals.due_value += parseFloat(client.due_value || 0);
                        grandTotals.balance += parseFloat(client.balance || 0);

                        const balanceClass = client.balance > 0 ? 'text-success' : client.balance < 0 ? 'text-danger' : 'text-muted';

                        tableHtml += `<tr>`;
                        tableHtml += `<td>${index + 1}</td>`;
                        tableHtml += `<td><span class="badge bg-secondary">${client.code || 'غير محدد'}</span></td>`;
                        tableHtml += `<td><span class="badge bg-info">${client.account_number || 'غير محدد'}</span></td>`;
                        tableHtml += `<td>
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="fw-bold">${client.name || 'غير محدد'}</div>
                                    ${client.currency_status ? `<small class="text-muted">${client.currency_status}</small>` : ''}
                                </div>
                            </div>
                        </td>`;
                        tableHtml += `<td>${client.branch ? `<span class="badge bg-success">${client.branch}</span>` : '<span class="text-muted">غير محدد</span>'}</td>`;
                        tableHtml += `<td>${client.employee ? `<span class="badge bg-warning">${client.employee}</span>` : '<span class="text-muted">غير محدد</span>'}</td>`;
                        tableHtml += `<td><span class="fw-bold">${formatNumber(client.balance_before)}</span></td>`;
                        tableHtml += `<td><span class="text-success fw-bold">${formatNumber(client.total_sales)}</span></td>`;
                        tableHtml += `<td><span class="text-danger fw-bold">${formatNumber(client.total_returns)}</span></td>`;
                        tableHtml += `<td><span class="text-info fw-bold">${formatNumber(client.net_sales)}</span></td>`;
                        tableHtml += `<td><span class="text-primary fw-bold">${formatNumber(client.total_payments)}</span></td>`;
                        tableHtml += `<td><span class="text-warning fw-bold">${formatNumber(client.due_value)}</span></td>`;
                        tableHtml += `<td><span class="${balanceClass} fw-bold">${formatNumber(client.balance)}</span></td>`;
                        tableHtml += `</tr>`;
                    });
                    ...
                    tableHtml += `
                        <tr class="table-grand-total">
                            ...
                            <td class="fw-bold">${formatNumber(grandTotals.balance_before)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.total_sales)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.total_returns)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.net_sales)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.total_payments)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.due_value)}</td>
                            <td class="fw-bold">${formatNumber(grandTotals.balance)}</td>
                        </tr>
                    `;
                } else {
                    tableHtml = `
            ...
            function exportToExcel() {
                ...
                const fileName = `تقرير_أرصدة_العملاء_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
