                ...
                <div class="col-md-8">
                    ...
                    <nav class="breadcrumb-custom">
                        <ol class="breadcrumb mb-0">
                            ...
                            <li class="breadcrumb-item active">تقرير مشتريات المنتجات</li>
                        </ol>
                    </nav>
                </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-3 col-md-6">
                            ...
                            <select name="invoice_type" id="invoice_type" class="form-control select2">
                                ...
                                <option value="City Notice">إشعار دائن</option>
                            </select>
                        </div>

                        <div class="col-lg-3 col-md-6">
                            <label class="form-label-modern">حالة الاستلام</label>
                            <select name="receiving_status" id="receiving_status" class="form-control select2">
                                <option value="">الكل</option>
                        ...
                        <div class="col-lg-3 col-md-6">
                            ...
                            <select name="report_type" id="report_type" class="form-control select2">
                                ...
                                <option value="purchase_manager">مدير مشتريات</option>
        ...
        <div class="card-modern fade-in" id="reportContainer" style="position: relative;">
            ...
            <div class="card-header-modern">
                <h5 class="mb-0" id="reportTitle">
                    ...
                    تقرير مشتريات المنتجات من {{ $fromDate->format('d/m/Y') }} إلى {{ $toDate->format('d/m/Y') }}
                </h5>
            </div>
                    ...
                    <table class="table table-modern mb-0">
                        <thead>
                            <tr>
                                ...
                                <th><i class="fas fa-tag me-2"></i>نوع الفاتورة</th>
                                ...
                                <th><i class="fas fa-check-circle me-2"></i>حالة الاستلام</th>
                            </tr>
                        </thead>
        ...
        $(document).ready(function() {
           ...
           $('.select2').select2({
            ...
            function loadReportData() {
                ...
                $.ajax({
                    url: '{{ route('ReportsPurchases.byProductReportAjax') }}',
                    method: 'GET',
                    data: formData,
            ...
            function updateReportDisplay(data) {
                ...
                $('#reportTitle').html(`
                    <i class="fas fa-table me-2"></i>
                    تقرير مشتريات المنتجات من ${data.from_date} إلى ${data.to_date}
                `);
            ...
            function updateTableBody(groupedProducts, totals) {
                ...
                Object.keys(groupedProducts).forEach(productId => {
                    ...
                    items.forEach(item => {
                        const isReturn = (item.invoice.type === 'Return');
                        ...
                        switch(item.invoice.type) {
                            case 'invoice':
                                typeBadge = '<span class="badge bg-success"><i class="fas fa-file-invoice me-1"></i>فاتورة</span>';
                                break;
                            case 'Return':
                                typeBadge = '<span class="badge bg-danger"><i class="fas fa-undo me-1"></i>مرتجع</span>';
                                break;
                            case 'Requested':
                                typeBadge = '<span class="badge bg-info"><i class="fas fa-shopping-cart me-1"></i>أمر شراء</span>';
                                break;
                            case 'City Notice':
                                typeBadge = '<span class="badge bg-warning"><i class="fas fa-bell me-1"></i>إشعار دائن</span>';
                                break;
                            default:
                                typeBadge = '<span class="badge bg-secondary">غير محدد</span>';
                        }
                        tableHtml += `<td>${typeBadge}</td>`;

                        tableHtml += `<td>${formatDate(item.invoice.date)}</td>`;
                        tableHtml += `<td>${item.invoice.supplier ? item.invoice.supplier.trade_name || item.invoice.supplier.name : 'غير محدد'}</td>`;
                        ...
                        // حالة الاستلام
                        const receivingStatus = item.invoice.receiving_status || 'not_received';
                        let statusBadge = '';
                        switch(receivingStatus) {
                            case 'received':
                                statusBadge = '<span class="badge bg-success"><i class="fas fa-check me-1"></i>مستلم بالكامل</span>';
                                break;
                            case 'partially_received':
                                statusBadge = '<span class="badge bg-warning"><i class="fas fa-clock me-1"></i>مستلم جزئياً</span>';
                                break;
                            case 'not_received':
                                statusBadge = '<span class="badge bg-secondary"><i class="fas fa-hourglass me-1"></i>غير مستلم</span>';
                                break;
                            default:
                                statusBadge = '<span class="badge bg-secondary"><i class="fas fa-hourglass me-1"></i>غير مستلم</span>';
                        }
            ...
            function exportToExcel() {
                ...
                const today = new Date();
                const fileName = `تقرير_مشتريات_المنتجات_${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}.xlsx`;
