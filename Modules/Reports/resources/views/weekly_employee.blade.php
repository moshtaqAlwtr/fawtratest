...
<body>
    ...
    <div class="payment-summary">
        ...
        <div class="payment-summary-grid">
            <div class="payment-summary-item">
                <div class="payment-summary-label">إجمالي المبيعات (فواتير عادية)</div>
                <div class="payment-summary-value">
                    {{ number_format($total_normal_invoices, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item">
                <div class="payment-summary-label">إجمالي المرتجعات (فواتير مرتجعة)</div>
                <div class="payment-summary-value">
                    {{ number_format($total_returned_invoices, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item">
                <div class="payment-summary-label">صافي المبيعات بعد المرتجعات</div>
                <div class="payment-summary-value">
                    {{ number_format($net_sales, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item">
                <div class="payment-summary-label">إجمالي المدفوعات المستلمة</div>
                <div class="payment-summary-value">
                    {{ number_format($total_payments, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item">
                <div class="payment-summary-label">إجمالي سندات القبض</div>
                <div class="payment-summary-value">
                    {{ number_format($total_receipts, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item">
                <div class="payment-summary-label">إجمالي سندات الصرف</div>
                <div class="payment-summary-value">
                    {{ number_format($total_expenses, 2, '.', ',') }} ر.س
                </div>
            </div>
            <div class="payment-summary-item grand-total">
                <div class="payment-summary-label">صافي التحصيل النقدي الأسبوعي</div>
                <div class="payment-summary-value">
                    {{ number_format($net_collection, 2, '.', ',') }} ر.س
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="section">
        <div class="section-title">
            <span>جميع الأنشطة خلال الأسبوع (مرتبة حسب الوقت)</span>
            <span class="section-count">{{ $allActivities->count() }}</span>
        </div>
        @if ($allActivities->count() > 0)
            <div class="date-range">الفترة من {{ $startDate }} إلى {{ $endDate }}</div>
            <table>
                <thead>
                    <tr>
                        ...
                        <th class="col-15">التاريخ والوقت</th>
                        <th class="col-12">المرجع</th>
                        <th class="col-14">التفاصيل</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($allActivities as $activity)
                        <tr class="activity-{{ $activity['type'] }}">
                            ...
                            <td class="time nowrap">{{ \Carbon\Carbon::parse($activity['time'])->format('Y-m-d H:i') }}</td>
        ...
        @else
            <div class="no-data">لا يوجد أنشطة مسجلة خلال هذا الأسبوع</div>
        @endif
    </div>
