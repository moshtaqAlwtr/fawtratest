...
@section('content')
    <div class="content-body">
        <form id="invoice-form" action="{{ route('CityNotices.store') }}" method="post" onsubmit="return confirmSubmit(event)">
            @csrf
            @if ($errors->any())
                                ...
                                <div class="mt-2">
                                    <span class="badge badge-info">
                                        <i class="fa fa-info-circle"></i>
                                         اشعار مدين من الفاتورة : {{ $originalInvoice->invoice_number ?? $originalInvoice->id }}
                                    </span>
                                </div>
                            @endif
                        </div>
...
<script src="{{ asset('assets/js/invoice.js') }}"></script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ربط حدث تغيير المورد لإظهار كارد الرصيد
            const supplierSelect = document.getElementById('clientSelect');
            if (supplierSelect) {
                supplierSelect.addEventListener('change', function() {
                    if (window.showSupplierBalance) {
                        window.showSupplierBalance(this);
                    }
                });
            }
            ...
            window.confirmSubmit = function(event) {
                ...
                Swal.fire({
                    title: 'تأكيد الحفظ',
                    text: 'هل أنت متأكد من حفظ اشعار المشتريات؟',
                    icon: 'question',
            ...
            window.saveAsDraft = function() {
                Swal.fire({
                    title: 'حفظ كمسودة',
                    text: 'هل تريد حفظ اشعار مدين  المشتريات كمسودة؟',
                    icon: 'info',
                    showCancelButton: true,
                    confirmButtonText: 'نعم، احفظ',
                    cancelButtonText: 'إلغاء'
