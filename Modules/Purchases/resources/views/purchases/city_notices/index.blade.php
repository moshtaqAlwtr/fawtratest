...
@section('scripts')
                    ...
                    if (response.total === 0) {
                        $('#results-container').html(`
                            <div class="alert alert-info text-center" role="alert">
                                ...
                                <p class="mb-0">لا يوجد  اشعارات مدينة تطابق معايير البحث</p>
                            </div>
                        `);
                    }
                } else {
    ...
    function initializeEvents() {
        // أحداث الحذف
        $('.delete-city-notice').off('click').on('click', function(e) {
            e.preventDefault();
            if (isLoading) return;

            const noticeId = $(this).data('id');
            const noticeCode = $(this).closest('tr').find('td:nth-child(2)').text().trim();

            Swal.fire({
                title: 'تأكيد الحذف',
                text: `هل أنت متأكد من حذف الاشعار المدين ${noticeCode}؟`,
                icon: 'warning',
    ...
    // حذف الاشعار المدين
    function deleteCityNotice(noticeId) {
        if (isLoading) return;

        const row = $(`.delete-city-notice[data-id="${noticeId}"]`).closest('tr');
        row.css('opacity', '0.5');

        $.ajax({
            url: `/CityNotices/destroy/${noticeId}`,
            type: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function(response) {
                Swal.fire({
                    icon: 'success',
                    title: 'تم الحذف',
                    text: 'تم حذف الاشعار المدين بنجاح',
                    timer: 2000,
            ...
            error: function(xhr, status, error) {
                ...
                let errorMessage = 'حدث خطأ أثناء حذف الاشعار المدين';
