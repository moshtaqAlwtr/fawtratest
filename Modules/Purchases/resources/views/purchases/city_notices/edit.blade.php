...
@section('content')
    <div class="content-body">
        <form id="invoice-form" action="{{ route('CityNotices.update',$cityNotice->id) }}" method="post" onsubmit="return confirmSubmit(event)">
            @csrf
            @method('PUT')
            @if ($errors->any())
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div>
                            ...
                            <div class="mt-2">
                                <span class="badge badge-warning">
                                    ...
                                    تعديل اشعار مدين رقم: {{ $cityNotice->invoice_number ?? $cityNotice->id }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="d-flex">
                            ...
                            <div>
                                <a href="{{ route('CityNotices.index') }}" class="btn btn-outline-danger">
                                                ...
                                                <select class="form-control select2" id="clientSelect" name="supplier_id" required>
                                                    ...
                                                    @foreach ($suppliers as $supplier)
                                                        <option value="{{ $supplier->id }}"
                                                            ...
                                                            @if($cityNotice->supplier_id == $supplier->id) selected
                                                            ...
                                                            <div class="col-8">
                                                                <a href="#" class="text-decoration-none" style="color: inherit;">
                                                                    <h5 class="card-title mb-2" id="supplierName" style="font-weight: 600; color: #333;">
                                                                        {{ $suppliers->where('id', $cityNotice->supplier_id)->first()->trade_name ?? 'اسم المورد' }}
                                                                    </h5>
                                                            ...
                                                            <div class="col-4 text-left">
                                                                <div class="d-flex flex-column align-items-end">
                                                                    <span style="font-size: 1.8rem; font-weight: 700; color: #333;" id="supplierBalance">
                                                                        {{ number_format($suppliers->where('id', $cityNotice->supplier_id)->first()->account->balance ?? 0, 2) }}
                                                                    </span>
                            ...
                            <div class="card-body">
                                <div class="row add_item">
                                    <div class="col-12">
                                        <div class="form-group row">
                                            <div class="col-md-3">
                                                <span>رقم الاشعار المدين :</span>
                                            </div>
                                            <div class="col-md-8">
                                                <input type="text" class="form-control" name="invoice_number"
                                                    value="{{ old('invoice_number', $cityNotice->invoice_number) }}" readonly>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="date" name="date"
                                                    value="{{ old('date', $cityNotice->date) }}">
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="text" name="return_reason"
                                                    value="{{ old('return_reason', $cityNotice->return_reason) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table" id="items-table">
                                ...
                                <tbody>
                                    @if($cityNotice->items && count($cityNotice->items) > 0)
                                        @foreach($cityNotice->items as $index => $item)
                                            <tr class="item-row">
                                                <td style="width:18%">
                                ...
                                <tfoot id="tax-rows">
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-right">المبلغ الاشعار المدين</td>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control"
                                        value="{{ old('discount_amount', $cityNotice->discount_amount ?? 0) }}"
                                        min="0" step="0.01">
                                    <select name="discount_type" class="form-control">
                                        <option value="amount" @if(old('discount_type', $cityNotice->discount_type ?? 'amount') == 'amount') selected @endif>ريال</option>
                                        <option value="percentage" @if(old('discount_type', $cityNotice->discount_type ?? 'amount') == 'percentage') selected @endif>نسبة مئوية</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" name="adjustment_label" class="form-control"
                                        value="{{ old('adjustment_label', $cityNotice->adjustment_label ?? '') }}"
                                        placeholder="اسم التسوية (مثال: خصم نقدي)">
                                    <select name="adjustment_type" class="form-control">
                                        <option value="discount" @if(old('adjustment_type', $cityNotice->adjustment_type ?? 'discount') == 'discount') selected @endif>خصم</option>
                                        <option value="addition" @if(old('adjustment_type', $cityNotice->adjustment_type ?? 'discount') == 'addition') selected @endif>إضافة</option>
                                    </select>
                                    <input type="number" name="adjustment_value" step="0.01" class="form-control"
                                        value="{{ old('adjustment_value', $cityNotice->adjustment_value ?? 0) }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <input type="number" id="paid-amount-input" class="form-control"
                                        value="{{ old('advance_payment', $cityNotice->advance_payment ?? 0) }}"
                                        name="advance_payment" step="0.01" min="0" placeholder="المبلغ المدفوع">
                                    <select name="advance_payment_type" class="form-control">
                                        <option value="amount" @if(old('advance_payment_type', $cityNotice->advance_payment_type ?? 'amount') == 'amount') selected @endif>ريال</option>
                                        <option value="percentage" @if(old('advance_payment_type', $cityNotice->advance_payment_type ?? 'amount') == 'percentage') selected @endif>نسبة مئوية</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                                ...
                                <select class="form-control" id="methodSelect" name="shipping_tax_id">
                                    ...
                                    @foreach ($taxs as $tax)
                                        <option value="{{ $tax->id }}" data-rate="{{ $tax->tax }}"
                                            @if(old('shipping_tax_id', $cityNotice->shipping_tax_id ?? '') == $tax->id) selected @endif>
                                            {{ $tax->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control" name="shipping_cost" id="shipping"
                                    value="{{ old('shipping_cost', $cityNotice->shipping_cost ?? 0) }}"
                                    min="0" step="0.01">
                            </div>
                        </div>
                    </div>
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <textarea id="tinyMCE" name="notes" class="form-control" rows="6"
                        style="font-size: 1.05rem;">{{ old('notes', $cityNotice->notes ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body py-2">
                    <div class="d-flex justify-content-start" style="direction: rtl;">
                        <div class="form-check">
                            <input class="form-check-input payment-toggle" type="checkbox"
                                   ...
                                   @if(old('is_paid', $cityNotice->is_paid ?? 0)) checked @endif>
                    ...
                    <div class="full-payment-fields mt-3" style="display: {{ old('is_paid', $cityNotice->is_paid ?? 0) ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-md-4">
                                <label for="payment_method">وسيلة الدفع</label>
                                <select class="form-control" name="payment_method">
                                    <option value="">اختر وسيلة الدفع</option>
                                    <option value="cash" @if(old('payment_method', $cityNotice->payment_method ?? '') == 'cash') selected @endif>نقداً</option>
                                    <option value="credit_card" @if(old('payment_method', $cityNotice->payment_method ?? '') == 'credit_card') selected @endif>بطاقة ائتمان</option>
                                    <option value="bank_transfer" @if(old('payment_method', $cityNotice->payment_method ?? '') == 'bank_transfer') selected @endif>تحويل بنكي</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">رقم المعرف</label>
                                <input type="text" class="form-control" name="reference_number"
                                    value="{{ old('reference_number', $cityNotice->reference_number ?? '') }}">
                            </div>
                        </div>
                        <div class="alert alert-info mt-2">
                            <small>
                                <i class="fa fa-info-circle"></i>
                                عند اختيار "استرداد كامل" سيتم تعيين المبلغ المدفوع تلقائياً لكامل المبلغ الاشعار
                            </small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="d-flex justify-content-start" style="direction: rtl;">
                        <div class="form-check">
                            <input class="form-check-input toggle-check" type="checkbox" name="is_received" value="1"
                                @if(old('is_received', $cityNotice->is_received ?? 0)) checked @endif>
                            <label class="form-check-label">تم استلام البضاعة</label>
                        </div>
                    </div>

                    <div class="payment-fields mt-3" style="display: {{ old('is_received', $cityNotice->is_received ?? 0) ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-md-4">
                                <label class="form-label">تاريخ الاستلام</label>
                                <input type="date" class="form-control" name="received_date"
                                    value="{{ old('received_date', $cityNotice->received_date ?? '') }}">
                            </div>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // حساب المجاميع عند التحميل إذا كان هناك عناصر موجودة
            @if($cityNotice->items && count($cityNotice->items) > 0)
                setTimeout(() => {
                    calculateTotals();
                }, 500);
            ...
            window.confirmSubmit = function(event) {
                ...
                Swal.fire({
                    title: 'تأكيد التحديث',
                    text: 'هل أنت متأكد من حفظ التعديلات على اشعار المشتريات؟',
                    icon: 'question',
            ...
            window.saveAsDraft = function() {
                Swal.fire({
                    title: 'حفظ كمسودة',
                    text: 'هل تريد حفظ تعديلات اشعار مدين المشتريات كمسودة؟',
                    icon: 'info',
                    showCancelButton: true,
