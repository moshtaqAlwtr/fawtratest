    ...
    <div class="card status-card">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <!-- الجانب الأيمن - حالة العرض والرقم -->
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <h5 class="mb-0 fw-bold">عرض سعر شراء #{{ $purchaseQuotation->code }}</h5>

@if ($purchaseQuotation->purchaseOrder)
    رقم طلب  عرض السعر: {{ $purchaseQuotation->purchaseOrder->code }}
@endif

                    <div class="d-flex align-items-center gap-2">
                        @if ($purchaseQuotation->status == "Under Review")
                            <i class="fas fa-clock text-warning" style="font-size: 12px;"></i>
                            <span class="status-badge badge-under-review">تحت المراجعة</span>
                        @elseif ($purchaseQuotation->status == "approval")
                            <i class="fas fa-check-circle text-success" style="font-size: 12px;"></i>
                            <span class="status-badge badge-approved">معتمد</span>
                        @elseif ($purchaseQuotation->status == "disagree")
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    @if ($purchaseQuotation->status == "Under Review")
                        <!-- أزرار الموافقة والرفض للعروض تحت المراجعة -->
                        <button type="button" class="status-btn btn-approve" onclick="approveQuotation()">
                            <i class="fas fa-check"></i>
                            <span>موافقة</span>
                        </button>

                        <button type="button" class="status-btn btn-reject" onclick="rejectQuotation()">
                            <i class="fas fa-times"></i>
                            <span>رفض</span>
                        </button>

                    @elseif ($purchaseQuotation->status == "approval")
                        <!-- أزرار للعروض المعتمدة -->
                        <div class="dropdown">
                            <button class="status-btn btn-convert pulse-effect" type="button" data-bs-toggle="dropdown">
                                <i class="fas fa-exchange-alt"></i>
                                <span>تحويل إلى أمر شراء</span>
                            </button>
                            <ul class="dropdown-menu supplier-dropdown py-1 shadow-lg">
                                @forelse($suppliers as $supplier)
                                    <li>
                                        <button type="button" class="dropdown-item d-flex align-items-center"
                                                onclick="convertToPurchaseOrder({{ $supplier->id }}, '{{ $supplier->trade_name }}')">
    ...
    <div class="card">
        <div class="card-title p-2 d-flex align-items-center gap-2">
            ...
            <!-- زر نسخ -->
            <button type="button" class="btn btn-outline-info btn-sm" onclick="copyQuotation()">
                نسخ <i class="fa fa-copy ms-1"></i>
            </button>
            <div class="vr"></div>

            <!-- زر حذف -->
            <button type="button" class="btn btn-outline-danger btn-sm" onclick="deleteQuotation()">
                            ...
                            <div class="tab-pane fade show active" style="background: lightslategray; min-height: 100vh; padding: 20px;">
                                <div class="card shadow preview-card" style="max-width: 600px; margin: 20px auto;">
                                    <div class="card-body bg-white p-4" style="min-height: 400px; overflow: auto;">
                                        @include('purchases::purchases.view_purchase_price.pdf', ['purchaseQuotation' => $purchaseQuotation])
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="tab-pane" id="items" role="tabpanel">
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="7" class="text-start fw-bold">الإجمالي</td>
                                    <td class="fw-bold">{{ number_format($purchaseQuotation->grand_total, 2) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
    ...
    <form id="approveForm" action="{{ route('pricesPurchase.approve', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" id="approveNote" name="note">
    </form>

    <form id="rejectForm" action="{{ route('pricesPurchase.reject', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
    </form>

    <form id="cancelApprovalForm" action="{{ route('pricesPurchase.cancelApproval', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
    </form>

    <form id="undoRejectionForm" action="{{ route('pricesPurchase.undoRejection', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
    </form>

    <form id="convertForm" action="{{ route('pricesPurchase.convertToPurchaseOrder', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" id="supplierId" name="supplier_id">
    </form>

    <form id="deleteForm" action="{{ route('pricesPurchase.destroy', $purchaseQuotation->id) }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
    </form>
@endsection
...
@section('scripts')
    ...
    <script>
        // دالة الموافقة على عرض السعر
        function approveQuotation() {
            Swal.fire({
                title: 'تأكيد الموافقة',
                text: 'هل أنت متأكد من الموافقة على عرض السعر رقم {{ $purchaseQuotation->code }}؟',
                icon: 'question',
        ...
        function rejectQuotation() {
            Swal.fire({
                title: 'تأكيد الرفض',
                text: 'هل أنت متأكد من رفض عرض السعر رقم {{ $purchaseQuotation->code }}؟',
                icon: 'warning',
                showCancelButton: true,
        ...
        // دالة تحويل إلى أمر شراء
        function convertToPurchaseOrder(supplierId, supplierName) {
            Swal.fire({
                title: 'تحويل إلى أمر شراء',
                text: `سيتم تحويل عرض السعر إلى أمر شراء للمورد: ${supplierName}`,
                icon: 'info',
                showCancelButton: true,
        ...
        function cancelApproval() {
            Swal.fire({
                ...
                text: 'هل أنت متأكد من إلغاء الموافقة على عرض السعر؟',
                icon: 'warning',
                showCancelButton: true,
        ...
        function undoRejection() {
            Swal.fire({
                ...
                text: 'هل تريد إعادة عرض السعر إلى حالة "تحت المراجعة"؟',
                icon: 'question',
                showCancelButton: true,
        ...
        function deleteQuotation() {
            Swal.fire({
                title: 'حذف عرض السعر',
                text: 'هل أنت متأكد من حذف عرض السعر رقم "{{ $purchaseQuotation->code }}"؟ هذا الإجراء لا يمكن التراجع عنه!',
                icon: 'error',
                showCancelButton: true,
                confirmButtonColor: '#dc3545',
                cancelButtonColor: '#6c757d',
        ...
        function copyQuotation() {
            Swal.fire({
                title: 'نسخ عرض السعر',
                text: 'سيتم إنشاء نسخة جديدة من عرض السعر',
                icon: 'info',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    // توجيه لصفحة إنشاء عرض سعر جديد بالبيانات المنسوخة
                    window.location.href = "{{ route('pricesPurchase.create') }}?copy_from={{ $purchaseQuotation->id }}";
                }
            });
        }
        ...
        function sendToSupplierEmail() {
            Swal.fire({
                ...
                text: 'سيتم إرسال عرض السعر إلى بريد المورد الإلكتروني',
                icon: 'info',
                showCancelButton: true,
                confirmButtonColor: '#ffc107',
                cancelButtonColor: '#6c757d',
                ...
                if (result.isConfirmed) {
                    Swal.fire('تم الإرسال!', 'تم إرسال عرض السعر بنجاح', 'success');
                }
            });
        }
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="process" class="form-control">
                                ...
                                <option value="مراجعة عرض السعر">مراجعة عرض السعر</option>
                                <option value="تعديل الأسعار">تعديل الأسعار</option>
                                <option value="التواصل مع المورد">التواصل مع المورد</option>
                                <option value="مناقشة شروط الدفع">مناقشة شروط الدفع</option>
                                <option value="متابعة الموافقة">متابعة الموافقة</option>
        ...
        function saveNoteToDatabase(noteData) {
            ...
            fetch(`{{ route('pricesPurchase.addNote', $purchaseQuotation->id) }}`, {
                method: 'POST',
                body: formData
            })
...
function viewAllNotes() {
    ...
    fetch(`{{ route('pricesPurchase.getNotes', $purchaseQuotation->id) }}`)
    .then(response => {
        if (!response.ok) {
    ...
    .catch(error => {
        ...
        function deleteNote(noteId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('pricesPurchase.deleteNote', '') }}/${noteId}`, {
                        method: 'DELETE',
                        headers: {
