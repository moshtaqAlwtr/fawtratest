...
@section('content')
    <div class="content-body">
        <form id="invoice-form" action="{{ route('ReturnsInvoice.update', $returnInvoice->id) }}" method="post" onsubmit="return confirmSubmit(event)">
            @csrf
            @method('PUT')
            @if ($errors->any())
            ...
            @if($returnInvoice->original_invoice_id)
                <input type="hidden" name="original_invoice_id" value="{{ $returnInvoice->original_invoice_id }}">
                <input type="hidden" name="reference_id" value="{{ $returnInvoice->reference_id }}">
            @endif
                            ...
                            @if($returnInvoice->original_invoice_id)
                                <div class="mt-2">
                                    <span class="badge badge-info">
                                        ...
                                        مرتجع من الفاتورة رقم: {{ $returnInvoice->originalInvoice->invoice_number ?? $returnInvoice->original_invoice_id }}
                                    </span>
                                </div>
                            @endif
                            <div class="mt-2">
                                <span class="badge badge-secondary">
                                    ...
                                    رقم المرتجع: {{ $returnInvoice->invoice_number }}
                                </span>
                            </div>
                        </div>
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                <select class="form-control select2" id="clientSelect" name="supplier_id" required
                                                    @if($returnInvoice->original_invoice_id) disabled @endif>
                                                    <option value="">اختر المورد</option>
                                                    @foreach ($suppliers as $supplier)
                                                        <option value="{{ $supplier->id }}"
                                                            data-balance="{{ $supplier->account->balance ?? 0 }}"
                                                            @if($returnInvoice->supplier_id == $supplier->id) selected
                                                ...
                                                @if($returnInvoice->original_invoice_id)
                                                    <input type="hidden" name="supplier_id" value="{{ $returnInvoice->supplier_id }}">
                                                @endif
                                            </div>
                                            <div class="col-md-4">
                                        ...
                                        <!-- كارد رصيد المورد -->
                                        <div class="row" id="supplierBalanceCard" style="display: block;">
                                            <div class="col-12">
                                                            ...
                                                            <div class="col-8">
                                                                <a href="#" class="text-decoration-none" style="color: inherit;">
                                                                    <h5 class="card-title mb-2" id="supplierName" style="font-weight: 600; color: #333;">
                                                                        {{ $returnInvoice->supplier->trade_name ?? 'غير محدد' }}
                                                                    </h5>
                                                            ...
                                                            <div class="col-4 text-left">
                                                                <div class="d-flex flex-column align-items-end">
                                                                    <span style="font-size: 1.8rem; font-weight: 700; color: #333;" id="supplierBalance">
                                                                        {{ number_format($returnInvoice->supplier->account->balance ?? 0, 2) }}
                                                                    </span>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input type="text" class="form-control" name="invoice_number"
                                                    value="{{ $returnInvoice->invoice_number }}" readonly>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="date" name="date"
                                                    value="{{ old('date', $returnInvoice->date) }}">
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="text" name="return_reason"
                                                    value="{{ old('return_reason', $returnInvoice->return_reason) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table" id="items-table">
                                ...
                                <tbody>
                                    @if($returnInvoice->items && count($returnInvoice->items) > 0)
                                        @foreach($returnInvoice->items as $index => $item)
                                            <tr class="item-row">
                                                <td style="width:18%">
                                ...
                                <tfoot id="tax-rows">
                                    ...
                                    <tr>
                                        ...
                                        <td><span id="subtotal">{{ number_format($returnInvoice->subtotal ?? 0, 2) }}</span> {!! $currencySymbol !!}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="7" class="text-right">مجموع الخصومات</td>
                                        <td><span id="total-discount">{{ number_format($returnInvoice->total_discount ?? 0, 2) }}</span> {!! $currencySymbol !!}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td><span id="shipping-cost">{{ number_format($returnInvoice->shipping_cost ?? 0, 2) }}</span> {!! $currencySymbol !!}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="7" class="text-right">المبلغ المرتجع</td>
                                        <td><span id="refund-amount">{{ number_format($returnInvoice->refund_amount ?? 0, 2) }}</span> {!! $currencySymbol !!}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="7" class="text-right">المجموع الكلي</td>
                                        <td><span id="grand-total">{{ number_format($returnInvoice->total_amount ?? 0, 2) }}</span> {!! $currencySymbol !!}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control"
                                        value="{{ old('discount_amount', $returnInvoice->discount_amount ?? 0) }}"
                                        min="0" step="0.01">
                                    <select name="discount_type" class="form-control">
                                        <option value="amount" @if(old('discount_type', $returnInvoice->discount_type ?? 'amount') == 'amount') selected @endif>ريال</option>
                                        <option value="percentage" @if(old('discount_type', $returnInvoice->discount_type ?? 'amount') == 'percentage') selected @endif>نسبة مئوية</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" name="adjustment_label" class="form-control"
                                        value="{{ old('adjustment_label', $returnInvoice->adjustment_label ?? '') }}"
                                        placeholder="اسم التسوية (مثال: خصم نقدي)">
                                    <select name="adjustment_type" class="form-control">
                                        <option value="discount" @if(old('adjustment_type', $returnInvoice->adjustment_type ?? 'discount') == 'discount') selected @endif>خصم</option>
                                        <option value="addition" @if(old('adjustment_type', $returnInvoice->adjustment_type ?? 'discount') == 'addition') selected @endif>إضافة</option>
                                    </select>
                                    <input type="number" name="adjustment_value" step="0.01" class="form-control"
                                        value="{{ old('adjustment_value', $returnInvoice->adjustment_value ?? 0) }}"
                    ...
                    <div id="section-deposit" class="tab-section d-none">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    <input type="number" id="paid-amount-input" class="form-control"
                                        value="{{ old('advance_payment', $returnInvoice->advance_payment ?? 0) }}"
                                        name="advance_payment" step="0.01" min="0" placeholder="المبلغ المدفوع">
                                    <select name="advance_payment_type" class="form-control">
                                        <option value="amount" @if(old('advance_payment_type', $returnInvoice->advance_payment_type ?? 'amount') == 'amount') selected @endif>ريال</option>
                                        <option value="percentage" @if(old('advance_payment_type', $returnInvoice->advance_payment_type ?? 'amount') == 'percentage') selected @endif>نسبة مئوية</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                                ...
                                <select class="form-control" id="methodSelect" name="shipping_tax_id">
                                    ...
                                    @foreach ($taxs as $tax)
                                        <option value="{{ $tax->id }}" data-rate="{{ $tax->tax }}"
                                            @if(old('shipping_tax_id', $returnInvoice->shipping_tax_id ?? '') == $tax->id) selected @endif>
                                            {{ $tax->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control" name="shipping_cost" id="shipping"
                                    value="{{ old('shipping_cost', $returnInvoice->shipping_cost ?? 0) }}"
                                    min="0" step="0.01">
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="section-documents" class="tab-section d-none">
                        ...
                        <div class="tab-content mt-3">
                            <div id="content-new-document" class="tab-pane active">
                                ...
                                @if($returnInvoice->attachments && count($returnInvoice->attachments) > 0)
                                    <div class="col-12 mb-3">
                                        <h6>المرفقات الحالية:</h6>
                                        <div class="row">
                                            @foreach($returnInvoice->attachments as $attachment)
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <textarea id="tinyMCE" name="notes" class="form-control" rows="6"
                        style="font-size: 1.05rem;">{{ old('notes', $returnInvoice->notes ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body py-2">
                    <div class="d-flex justify-content-start" style="direction: rtl;">
                        <div class="form-check">
                            <input class="form-check-input payment-toggle" type="checkbox"
                                   name="is_paid" value="1" id="full-payment-check"
                                   @if(old('is_paid', $returnInvoice->is_paid ?? 0)) checked @endif>
                    ...
                    <div class="full-payment-fields mt-3" style="display: {{ old('is_paid', $returnInvoice->is_paid ?? 0) ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-md-4">
                                <label for="payment_method">وسيلة الدفع</label>
                                <select class="form-control" name="payment_method">
                                    <option value="">اختر وسيلة الدفع</option>
                                    <option value="cash" @if(old('payment_method', $returnInvoice->payment_method ?? '') == 'cash') selected @endif>نقداً</option>
                                    <option value="credit_card" @if(old('payment_method', $returnInvoice->payment_method ?? '') == 'credit_card') selected @endif>بطاقة ائتمان</option>
                                    <option value="bank_transfer" @if(old('payment_method', $returnInvoice->payment_method ?? '') == 'bank_transfer') selected @endif>تحويل بنكي</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">رقم المعرف</label>
                                <input type="text" class="form-control" name="reference_number"
                                    value="{{ old('reference_number', $returnInvoice->reference_number ?? '') }}">
                            </div>
                        </div>
                    ...
                    <div class="d-flex justify-content-start" style="direction: rtl;">
                        <div class="form-check">
                            <input class="form-check-input toggle-check" type="checkbox" name="is_received" value="1"
                                @if(old('is_received', $returnInvoice->is_received ?? 0)) checked @endif>
                            <label class="form-check-label">تم استلام البضاعة</label>
                        </div>
                    </div>

                    <div class="payment-fields mt-3" style="display: {{ old('is_received', $returnInvoice->is_received ?? 0) ? 'block' : 'none' }};">
                        <div class="row">
                            <div class="col-md-4">
                                <label class="form-label">تاريخ الاستلام</label>
                                <input type="date" class="form-control" name="received_date"
                                    value="{{ old('received_date', $returnInvoice->received_date ?? '') }}">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateTotals() {
                ...
                document.getElementById('refund-amount').textContent = (parseFloat(refundAmountInput?.value || grandTotal)).toFixed(2);
            }
            ...
            window.confirmSubmit = function(event) {
                ...
                Swal.fire({
                    title: 'تأكيد التحديث',
                    text: 'هل أنت متأكد من تحديث مرتجع المشتريات؟',
                    icon: 'question',
            ...
            window.saveAsDraft = function() {
                Swal.fire({
                    title: 'حفظ كمسودة',
                    text: 'هل تريد حفظ التغييرات كمسودة؟',
                    icon: 'info',
                    showCancelButton: true,
            ...
            // ربط أحداث الخصومات والشحن لإعادة الحساب
            document.querySelectorAll('input[name="discount_amount"], select[name="discount_type"], input[name="shipping_cost"], input[name="adjustment_value"], select[name="adjustment_type"]').forEach(input => {
