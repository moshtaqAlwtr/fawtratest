...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">عرض مرتجع الشراء</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">الرئيسية</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('ReturnsInvoice.index') }}">مرتجعات الشراء</a>
                            </li>
        ...
        <div class="row align-items-center justify-content-between">
            ...
            <div class="col-lg-8 text-end">
                <div class="row text-end">
                    <div class="col-md-6">
                        <h3 class="">
                            مرتجع الشراء #{{ $purchaseInvoiceReturn->code }}
                        </h3>
                        <p class="mb-1">
                            <strong>المورد:</strong> {{ $purchaseInvoiceReturn->supplier->trade_name }}
                        </p>
                        <p class="mb-1">
                            <strong>تاريخ المرتجع:</strong> {{ $purchaseInvoiceReturn->created_at->format('Y-m-d') }}
                        </p>
                        <p class="mb-1">
                            <strong>الفاتورة الأصلية:</strong> #{{ $purchaseInvoiceReturn->reference_id??'-' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4 d-flex flex-column align-items-end gap-2">
                ...
                <button class="btn btn-success btn-bg action-btn" onclick="printInvoice()">
                    <i class="fas fa-print ms-1"></i> طباعة المرتجع
                </button>

                <!-- الحالة -->
                <div class="d-flex flex-wrap gap-2 justify-content-end">
        @if ($purchaseInvoiceReturn->type == 'Return')
            <span class="status-badge bg-success">مرتجع</span>

        @endif

        @switch($purchaseInvoiceReturn->receiving_status)
            @case('received')
        ...
        <div class="card-body">
            ...
            <div class="tab-content p-3">
                                ...
                                <div class="card shadow preview-card" style="max-width: 600px; margin: 20px auto;">
                                    <div class="card-body bg-white p-4" style="min-height: 400px; overflow: auto;">
                                        <div id="print-section"
                                            ...
                                            @include('purchases::purchases.returns.pdf', [
                                                'purchaseInvoiceReturn' => $purchaseInvoiceReturn,
                                            ])
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="tab-pane" id="items" role="tabpanel">
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @foreach ($purchaseInvoiceReturn->items as $item)
                                    <tr>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="fw-bold">{{ number_format($purchaseInvoiceReturn->grand_total, 2) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div class="tab-pane" id="payments" role="tabpanel">
                    @if ($purchaseInvoiceReturn->payments->count() > 0)
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @foreach ($purchaseInvoiceReturn->payments as $payment)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                    ...
                    @else
                        <div class="alert alert-info text-center">
                            ...
                            لا توجد مدفوعات لهذا المرتجع حتى الآن
                        </div>
                    @endif
                </div>
    ...
    <form id="deleteForm" action="{{ route('invoicePurchases.destroy', $purchaseInvoiceReturn->id) }}" method="POST"
        style="display: none;">
        @csrf
        @method('DELETE')
    </form>
    ...
    <script>
        ...
        function deleteInvoice() {
            Swal.fire({
                ...
                text: 'هل أنت متأكد من حذف مرتجع الشراء رقم "{{ $purchaseInvoiceReturn->code }}"؟ هذا الإجراء لا يمكن التراجع عنه!',
                icon: 'error',
        ...
        function copyInvoice() {
            Swal.fire({
                title: 'نسخ مرتجع الشراء',
                text: 'سيتم إنشاء نسخة جديدة من مرتجع الشراء',
                icon: 'info',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    // توجيه لصفحة إنشاء مرتجع شراء جديد بالبيانات المنسوخة
                    window.location.href =
                        "{{ route('invoicePurchases.create') }}?copy_from={{ $purchaseInvoiceReturn->id }}";
                }
            });
        }
        ...
        function assignCostCenter() {
            Swal.fire({
                ...
                text: 'سيتم توجيهك لصفحة تعيين مراكز التكلفة لهذا المرتجع',
                icon: 'info',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href =
                        "{{ route('invoicePurchases.assignCostCenter', $purchaseInvoiceReturn->id) }}";
                }
            });
        }
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="process" class="form-control">
                                ...
                                <option value="مراجعة المرتجع">مراجعة المرتجع</option>
        ...
        function saveNoteToDatabase(noteData) {
            ...
            fetch(`{{ route('invoicePurchases.addNote', $purchaseInvoiceReturn->id) }}`, {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
        ...
        function viewAllNotes() {
            ...
            fetch(`{{ route('invoicePurchases.getNotes', $purchaseInvoiceReturn->id) }}`)
                .then(response => {
                    if (!response.ok) {
        ...
        function printInvoice() {
            ...
            printWindow.document.write('<html><head><title>مرتجع شراء</title>');
