{{-- ملف: resources/views/purchases/returns/partials/pagination.blade.php --}}

@if ($cityNotices->hasPages())
    <div class="d-flex justify-content-between align-items-center mt-3">
        <div class="pagination-info text-muted">
            عرض {{ $cityNotices->firstItem() }} إلى {{ $cityNotices->lastItem() }} من {{ $cityNotices->total() }} مرتجع
        </div>
    ...
    <div class="row mt-2">
        <div class="col-md-6">
            <small class="text-muted">
                ...
                ({{ $cityNotices->total() }} مرتجع إجمالي)
            </small>
        </div>
    ...
    <div class="text-center text-muted mt-3">
        <small>
            @if($cityNotices->total() > 0)
                عرض جميع النتائج ({{ $cityNotices->total() }} مرتجع)
            @else
