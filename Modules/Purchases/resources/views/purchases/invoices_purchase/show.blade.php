    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">عرض فاتورة الشراء</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">الرئيسية</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('invoicePurchases.index') }}">فواتير الشراء</a>
                            </li>
...
<div class="invoice-header">
            ...
            <div class="row text-end">
                ...
                فاتورة الشراء #{{ $purchaseInvoice->code }}
                    ...
                    <p class="mb-1">
                        ...
                        <strong>المورد:</strong> {{ $purchaseInvoice->supplier->trade_name }}
                    </p>
                    <p class="mb-1">

                        <strong>تاريخ الفاتورة:</strong> {{ $purchaseInvoice->created_at->format('Y-m-d') }}
                    </p>
                </div>
    ...
    <!-- الحالات: الدفع والاستلام -->
    <div class="d-flex flex-wrap gap-2 justify-content-end">
        @if ($purchaseInvoice->payment_status == 'paid')
            <span class="status-badge bg-success">تم الدفع</span>
        @elseif ($purchaseInvoice->payment_status == 'unpaid')
            <span class="status-badge bg-danger">لم يتم الدفع</span>
        @elseif ($purchaseInvoice->payment_status == 'partially_paid')
            <span class="status-badge bg-warning">تم الدفع جزئياً</span>
        @endif

        @switch($purchaseInvoice->receiving_status)
            @case('received')
    ...
    <div class="card">
        <div class="card-title p-2 d-flex align-items-center gap-2">
            <!-- زر تعديل -->
            <a href="{{ route('invoicePurchases.edit', $purchaseInvoice->id) }}" class="btn btn-outline-primary btn-sm">
            ...
            <a href="{{ route('PaymentSupplier.createPurchase', $purchaseInvoice->id) }}" class="btn btn-outline-primary btn-sm">
                اضافة عملية دفع <i class="fa fa-credit-card ms-1"></i>
            </a>
            <!-- زر المطبوعات -->
            <div class="dropdown">ئ
            ...
            @if($purchaseInvoice->type == 'invoice')
            <div class="dropdown">
                <button class="btn btn-outline-success btn-sm" type="button" data-bs-toggle="dropdown">
                    مرتجع <i class="fa fa-paperclip ms-1"></i>
                </button>
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{ route('ReturnsInvoice.createFromInvoice', $purchaseInvoice->id) }}">
                            <i class="fa fa-plus me-2 text-success"></i> إنشاء مرتجع</a>
                    </li>
                    <li><a class="dropdown-item" href="{{ route('CreditNotes.createFromInvoice', $purchaseInvoice->id) }}') }}">
                            <i class="fa fa-list me-2 text-info"></i> اشعار مدين</a>
                    </li>
                </ul>
            </div>
            @endif
                                ...
                                <div class="card shadow preview-card" style="max-width: 600px; margin: 20px auto;">
                                    <div class="card-body bg-white p-4" style="min-height: 400px; overflow: auto;">
                                        <div id="print-section"
                                            ...
                                            @include('purchases::purchases.invoices_purchase.pdf', [
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>الخصم</th>
                                    <th>الضريبة</th>
                                    @if ($purchaseInvoice->shipping_cost > 0)
                                        <th>الشحن</th>
                                    @endif
                                    <th>الإجمالي</th>
                                </tr>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="{{ $purchaseInvoice->shipping_cost > 0 ? '7' : '6' }}"
                                        class="text-start fw-bold">الإجمالي</td>
                                    <td class="fw-bold">{{ number_format($purchaseInvoice->grand_total, 2) }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div class="tab-pane" id="payments" role="tabpanel">
                    ...
                    @else
                        <div class="alert alert-info text-center">
                            ...
                            لا توجد مدفوعات لهذه الفاتورة حتى الآن
                        </div>
                    @endif
                </div>
    ...
    <form id="deleteForm" action="{{ route('invoicePurchases.destroy', $purchaseInvoice->id) }}" method="POST"
        style="display: none;">
        @csrf
        @method('DELETE')
    </form>
...
@section('scripts')
    ...
    <script>
        ...
        function deleteInvoice() {
            Swal.fire({
                ...
                text: 'هل أنت متأكد من حذف فاتورة الشراء رقم "{{ $purchaseInvoice->code }}"؟ هذا الإجراء لا يمكن التراجع عنه!',
                icon: 'error',
        ...
        function copyInvoice() {
            Swal.fire({
                title: 'نسخ فاتورة الشراء',
                text: 'سيتم إنشاء نسخة جديدة من فاتورة الشراء',
                icon: 'info',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    // توجيه لصفحة إنشاء فاتورة شراء جديدة بالبيانات المنسوخة
                    window.location.href =
                        "{{ route('invoicePurchases.create') }}?copy_from={{ $purchaseInvoice->id }}";
                }
            });
        }
        ...
        function assignCostCenter() {
            Swal.fire({
                ...
                text: 'سيتم توجيهك لصفحة تعيين مراكز التكلفة لهذه الفاتورة',
                icon: 'info',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    // توجيه لصفحة تعيين مراكز التكلفة
                    window.location.href =
                        "{{ route('invoicePurchases.assignCostCenter', $purchaseInvoice->id) }}";
                }
            });
        }
        ...
        function editPayment(paymentId) {
            // جلب بيانات الدفعة
            fetch(`/payment-supplier/${paymentId}/edit`)
                        ...
                        Swal.fire({
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label for="edit_amount" class="form-label text-start d-block">المبلغ:</label>
                                    <input type="number" id="edit_amount" class="form-control"
                                           value="${payment.amount}" step="0.01" min="0" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="edit_payment_method" class="form-label text-start d-block">طريقة الدفع:</label>
                                    <select id="edit_payment_method" class="form-control" required>
                                        <option value="cash" ${payment.payment_method === 'cash' ? 'selected' : ''}>نقداً</option>
                                        <option value="bank" ${payment.payment_method === 'bank' ? 'selected' : ''}>تحويل بنكي</option>
                                        <option value="check" ${payment.payment_method === 'check' ? 'selected' : ''}>شيك</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label for="edit_notes" class="form-label text-start d-block">الملاحظات:</label>
                                <textarea id="edit_notes" class="form-control" rows="3">${payment.notes || ''}</textarea>
                            </div>
                        `,
                            width: '500px',
                            showCancelButton: true,
        ...
        function updatePayment(paymentId, paymentData) {
            ...
            fetch(`/payment-supplier/${paymentId}`, {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        Swal.fire('تم التحديث!', 'تم تحديث الدفعة بنجاح', 'success')
                            .then(() => location.reload());
                    } else {
                        Swal.fire('خطأ!', data.message || 'حدث خطأ أثناء تحديث الدفعة', 'error');
                    }
                })
                .catch(error => {
                    ...
                    fetch(`/payment-supplier/${paymentId}`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire('خطأ!', data.message || 'حدث خطأ أثناء حذف الدفعة', 'error');
                            }
                        })
                        .catch(error => {
                            Swal.fire('خطأ!', 'حدث خطأ أثناء الاتصال بالخادم', 'error');
        ...
        // دالة إضافة ملاحظة أو مرفق (نفس الكود من أمر الشراء مع تعديل المسارات)
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="process" class="form-control">
                                ...
                                <option value="تسجيل الدفع">تسجيل الدفع</option>
                                <option value="تأكيد الاستلام">تأكيد الاستلام</option>
                                <option value="مراجعة البضائع">مراجعة البضائع</option>
        ...
        function saveNoteToDatabase(noteData) {
            ...
            fetch(`{{ route('invoicePurchases.addNote', $purchaseInvoice->id) }}`, {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.json())
        ...
        function viewAllNotes() {
            ...
            fetch(`{{ route('invoicePurchases.getNotes', $purchaseInvoice->id) }}`)
                .then(response => {
                    if (!response.ok) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const pulseButtons = document.querySelectorAll('.pulse-effect');
    ...
    <script>
        function printInvoice() {
            ...
            '<link rel="stylesheet" href="{{ asset('assets/admin/css/style.css') }}">'); // لو عندك ملف CSS خارجي
