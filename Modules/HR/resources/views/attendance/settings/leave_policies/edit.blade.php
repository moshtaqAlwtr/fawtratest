            ...
            <form id="editHolidayForm" method="POST" action="{{ route('leave_policy.update', $leave_policy->id) }}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $leave_policy->name) }}" required>
                    ...
                    <div class="col-md-12 mt-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3" placeholder="اكتب وصف للسياسة...">{{ old('description', $leave_policy->description) }}</textarea>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function hasChanges() {
            ...
            const originalEntries = [...originalData.entries()].sort();
            const currentEntries = [...currentData.entries()].sort();
        ...
        holidayTable.addEventListener('click', function (e) {
            if (e.target.classList.contains('remove-row') || e.target.closest('.remove-row')) {
                ...
                Swal.fire({
                    title: 'تأكيد الحذف',
                    text: 'هل تريد حذف هذا النوع من الإجازة؟',
                    icon: 'warning',
