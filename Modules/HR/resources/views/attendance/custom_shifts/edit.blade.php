...
@section('title', 'تعديل وردية متخصصة')
...
@section('content')
    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">تعديل وردية متخصصة</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="#">الرئيسية</a></li>
                            <li class="breadcrumb-item"><a href="{{ route('custom_shifts.index') }}">الورديات المتخصصة</a></li>
    ...
    <div class="content-body">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('custom_shifts.update', $customShift->id) }}" method="POST" id="editAttendanceForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $customShift->name) }}">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="is_main" id="is_main" class="form-control select2" required>
                                <option value="" disabled>اختر نوع الوردية</option>
                                <option value="1" {{ old('is_main', $customShift->is_main) == '1' ? 'selected' : '' }}>رئيسي</option>
                                <option value="0" {{ old('is_main', $customShift->is_main) == '0' ? 'selected' : '' }}>فرعي</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="shift_id" id="shift_id" class="form-control select2">
                                ...
                                @foreach ($shifts as $shift)
                                    <option value="{{ $shift->id }}" {{ old('shift_id', $customShift->shift_id) == $shift->id ? 'selected' : '' }}>{{ $shift->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label for="attendance-date" class="form-label">تاريخ البدء <span style="color: red">*</span></label>
                            <input type="date" name="from_date" class="form-control" value="{{ old('from_date', $customShift->from_date) }}">
                        </div>
                        <div class="col-md-6">
                            <label for="attendance-date" class="form-label">تاريخ الإنتهاء <span style="color: red">*</span></label>
                            <input type="date" name="to_date" class="form-control" value="{{ old('to_date', $customShift->to_date) }}">
                        </div>
                    </div>
                            ...
                            <div class="col-12">
                                <ul class="list-unstyled mb-0">
                                    <li class="d-inline-block mr-5">
                                        <fieldset>
                                            <div class="custom-control custom-radio">
                                                <input type="radio" class="custom-control-input" name="use_rules" id="customRadio1" {{ old('use_rules', $customShift->use_rules) == 'rules' ? 'checked' : '' }} value="rules">
                                    ...
                                    <li class="d-inline-block mr-5">
                                        <fieldset>
                                            <div class="custom-control custom-radio">
                                                <input type="radio" class="custom-control-input" name="use_rules" id="customRadio2" {{ old('use_rules', $customShift->use_rules) == 'employees' ? 'checked' : '' }} value="employees">
                    ...
                    <div id="employee" style="display: {{ old('use_rules', $customShift->use_rules) == 'employees' ? 'block' : 'none' }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="employee-select-employees" name="employee_id[]" class="form-control select2" multiple="multiple">
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ in_array($employee->id, old('employee_id', $customShift->employees->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $employee->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div id="rules" style="display: {{ old('use_rules', $customShift->use_rules) == 'rules' ? 'block' : 'none' }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="branch" name="branch_id" class="form-control select2">
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $customShift->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select id="department" name="department_id" class="form-control select2">
                                    ...
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}" {{ old('department_id', $customShift->department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select id="job_title" name="job_title_id" class="form-control select2">
                                    ...
                                    @foreach ($job_titles as $job_title)
                                        <option value="{{ $job_title->id }}" {{ old('job_title_id', $customShift->job_title_id) == $job_title->id ? 'selected' : '' }}>{{ $job_title->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="shift_rule_id" id="shift_rule_id" class="form-control select2">
                                    <option value="" disabled selected>اختر الوردية</option>
                                    @foreach ($shifts as $shift)
                                        <option value="{{ $shift->id }}" {{ old('shift_rule_id', $customShift->shift_rule_id) == $shift->id ? 'selected' : '' }}>{{ $shift->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="" selected>اختر الموظفين المستبعدين</label>
                                <select id="employee-select-rules" name="excluded_employees[]" class="form-control select2" multiple="multiple">
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ in_array($employee->id, old('excluded_employees', $customShift->excludedEmployees->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $employee->full_name }}
                                        </option>
                                    @endforeach
                                </select>
