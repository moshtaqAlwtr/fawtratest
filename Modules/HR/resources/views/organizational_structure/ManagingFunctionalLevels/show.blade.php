...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ $level->name }}</h2>
    ...
    <div class="card">
        <div class="card-title p-2">
            ...
            <a href="{{ route('ManagingJobTypes.edit',$level->id) }}" class="btn btn-outline-primary btn-sm waves-effect waves-light">تعديل <i class="fa fa-edit"></i></a>
            <a href="{{ route('ManagingJobTypes.updateStatus',$level->id) }}" class="btn btn-outline-{{ $level->status == 0 ? 'danger' : 'success' }} btn-sm waves-effect waves-light">
                {{ $level->status == 0 ? 'تعطيل' : 'تفعيل' }} <i class="fa {{ $level->status == 0 ? 'fa-ban' : 'fa-check' }}"></i></a>
        </div>
        <div class="card-body">
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" id="journal-entries-tab" data-toggle="tab" href="#journal-entries"
                        role="tab" aria-controls="journal-entries" aria-selected="false">الموطفين المعنين ({{ $level->employees->count() }})</a>
                </li>
            ...
            <div class="tab-content">
                                ...
                                <div class="row">
                                    <table class="table">
                                        <thead style="background: #f8f8f8">
                                            <tr>
                                                <th>معلومات نوع وظيفة</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <p><small>الوصف</small>: </p><span>{{ $level->description }}</span>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                ...
                <div class="tab-pane fade" id="journal-entries" role="tabpanel"
                    ...
                    <div class="card">
                        <div class="card-body">
                            @if ($level->employees->where('employee_type',1) !== null && !@empty($level->employees->where('employee_type',1)) && count($level->employees->where('employee_type',1)) > 0)
                                <table class="table">
                                    <thead style="background: #f8f8f8">
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($level->employees->where('employee_type',1) as $employee)
                                            <tr>
