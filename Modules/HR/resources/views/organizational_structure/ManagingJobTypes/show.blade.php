...
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ $type->name }}</h2>
    ...
    <div class="card">
        <div class="card-title p-2">
            ...
            <a href="{{ route('ManagingJobTypes.edit',$type->id) }}" class="btn btn-outline-primary btn-sm waves-effect waves-light">تعديل <i class="fa fa-edit"></i></a>
            <a href="{{ route('ManagingJobTypes.updateStatus',$type->id) }}" class="btn btn-outline-{{ $type->status == 0 ? 'danger' : 'success' }} btn-sm waves-effect waves-light">
                {{ $type->status == 0 ? 'تعطيل' : 'تفعيل' }} <i class="fa {{ $type->status == 0 ? 'fa-ban' : 'fa-check' }}"></i></a>
        </div>
        <div class="card-body">
            <ul class="nav nav-tabs" role="tablist">
                ...
                <li class="nav-item">
                    <a class="nav-link" id="journal-entries-tab" data-toggle="tab" href="#journal-entries"
                        role="tab" aria-controls="journal-entries" aria-selected="false">الموطفين المعنين ({{ $type->employees->count() }})</a>
                </li>
            ...
            <div class="tab-content">
                                ...
                                <div class="row">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <p><small>الوصف</small>: </p><span>{{ $type->description }}</span>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                ...
                <div class="tab-pane fade" id="journal-entries" role="tabpanel"
                    ...
                    <div class="card">
                        <div class="card-body">
                            @if ($type->employees->where('employee_type',1) !== null && !@empty($type->employees->where('employee_type',1)) && count($type->employees->where('employee_type',1)) > 0)
                                <table class="table">
                                    <thead style="background: #f8f8f8">
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($type->employees->where('employee_type',1) as $employee)
                                            <tr>
