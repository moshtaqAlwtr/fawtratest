...
@section('content')
    <div class="content-body">
        <form id="invoice-form" action="{{ route('questions.update', $quote->id) }}" method="post" onsubmit="return confirmUpdate(event)">
            @csrf
            @method('PUT')
            @if ($errors->any())
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        ...
                        <div class="d-flex">
                            <div class="btn-group mr-2" style="margin-left: 10px;">
                                ...
                                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="duplicateQuote()" title="تكرار عرض السعر">
                                ...
                                <div class="row add_item">
                                    <div class="col-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input type="text" class="form-control" name="quote_number" value="{{ $quote->quote_number ?? '' }}" readonly>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="date" name="quote_date" value="{{ old('quote_date', $quote->quote_date ?? date('Y-m-d')) }}">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <input class="form-control" type="date" name="expiry_date" value="{{ old('expiry_date', $quote->expiry_date ?? '') }}">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-6">
                                                <input class="form-control" type="text" name="terms" value="{{ old('terms', $quote->terms ?? '') }}">
                                            </div>
                                            <div class="col-md-2">
                                    ...
                                    <div class="col-12">
                                        <div class="form-group row">
                                            <div class="col-md-3">
                                                <input class="form-control" type="text" placeholder="عنوان إضافي" value="{{ old('additional_title', $quote->additional_title ?? '') }}">
                                            </div>
                                            <div class="col-md-8">
                                                <div class="input-group">
                                                    <input class="form-control" type="text" placeholder="بيانات إضافية" value="{{ old('additional_data', $quote->additional_data ?? '') }}">
                                    ...
                                    @if(isset($quote->items) && count($quote->items) > 0)
                                        @foreach($quote->items as $index => $quoteItem)
                                            <tr class="item-row">
                                                    ...
                                                    <select name="items[{{ $index }}][product_id]" class="form-control product-select" required>
                                                        ...
                                                        @foreach ($items as $item)
                                                            <option value="{{ $item->id }}"
                                                                ...
                                                                {{ $quoteItem->product_id == $item->id ? 'selected' : '' }}>
                                                                {{ $item->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <td data-label="الوصف">
                                                    <input type="text" name="items[{{ $index }}][description]" class="form-control item-description" placeholder="أدخل الوصف" value="{{ $quoteItem->description ?? '' }}">
                                                </td>
                                                <td data-label="الكمية">
                                                    <input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity" value="{{ $quoteItem->quantity ?? 1 }}" min="1" required>
                                                </td>
                                                <td data-label="السعر">
                                                    <input type="number" name="items[{{ $index }}][unit_price]" class="form-control price" step="0.01" required placeholder="0.00" value="{{ $quoteItem->unit_price ?? '' }}">
                                                </td>
                                                <td data-label="الخصم">
                                                    <div class="input-group">
                                                        <input type="number" name="items[{{ $index }}][discount]" class="form-control discount-value" value="{{ $quoteItem->discount ?? 0 }}" min="0" step="0.01">
                                                        <select name="items[{{ $index }}][discount_type]" class="form-control discount-type">
                                                            <option value="amount" {{ ($quoteItem->discount_type ?? 'amount') == 'amount' ? 'selected' : '' }}>ريال</option>
                                                            <option value="percentage" {{ ($quoteItem->discount_type ?? '') == 'percentage' ? 'selected' : '' }}>نسبة %</option>
                                                        </select>
                                                    </div>
                                                </td>
                                                <td data-label="الضريبة 1">
                                                    <div class="input-group">
                                                        <select name="items[{{ $index }}][tax_1]" class="form-control tax-select" data-target="tax_1" onchange="updateHiddenInput(this)">
                                                            ...
                                                            @foreach ($taxs as $tax)
                                                                <option value="{{ $tax->tax }}"
                                                                    ...
                                                                    data-name="{{ $tax->name }}"
                                                                    data-type="{{ $tax->type }}"
                                                                    {{ isset($quoteItem->tax_1_id) && $quoteItem->tax_1_id == $tax->id ? 'selected' : '' }}>
                                                                    {{ $tax->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        <input type="hidden" name="items[{{ $index }}][tax_1_id]" value="{{ $quoteItem->tax_1_id ?? '' }}">
                                                    </div>
                                                </td>

                                                <td data-label="الضريبة 2">
                                                    <div class="input-group">
                                                        <select name="items[{{ $index }}][tax_2]" class="form-control tax-select" data-target="tax_2" onchange="updateHiddenInput(this)">
                                                            <option value="">لا يوجد</option>
                                                            @foreach ($taxs as $tax)
                                                                <option value="{{ $tax->tax }}"
                                                                    data-id="{{ $tax->id }}"
                                                                    data-name="{{ $tax->name }}"
                                                                    data-type="{{ $tax->type }}"
                                                                    {{ isset($quoteItem->tax_2_id) && $quoteItem->tax_2_id == $tax->id ? 'selected' : '' }}>
                                                                    {{ $tax->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        <input type="hidden" name="items[{{ $index }}][tax_2_id]" value="{{ $quoteItem->tax_2_id ?? '' }}">
                                                    </div>
                                                </td>
                    ...
                    <div id="section-discount" class="tab-section">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" name="discount_amount" class="form-control" value="{{ old('discount_amount', $quote->discount_amount ?? 0) }}" min="0" step="0.01">
                                    <select name="discount_type" class="form-control">
                                        <option value="amount" {{ ($quote->discount_type ?? 'amount') == 'amount' ? 'selected' : '' }}>ريال</option>
                                        <option value="percentage" {{ ($quote->discount_type ?? '') == 'percentage' ? 'selected' : '' }}>نسبة مئوية</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <select class="form-control" id="methodSelect" name="tax_type">
                                    <option value="1" {{ ($quote->tax_type ?? '') == '1' ? 'selected' : '' }}>القيمة المضافة (15%)</option>
                                    <option value="2" {{ ($quote->tax_type ?? '') == '2' ? 'selected' : '' }}>صفرية</option>
                                    <option value="3" {{ ($quote->tax_type ?? '') == '3' ? 'selected' : '' }}>معفاة</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">تكلفة الشحن</label>
                                <input type="number" class="form-control" name="shipping_cost" id="shipping" value="{{ old('shipping_cost', $quote->shipping_cost ?? 0) }}" min="0" step="0.01">
                            </div>
                        </div>
                    </div>
            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <textarea id="tinyMCE" name="notes" class="form-control" rows="6" style="font-size: 1.05rem;">{{ old('notes', $quote->notes ?? '') }}</textarea>
                </div>
            </div>
        </form>
    </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            console.log('صفحة تعديل عرض السعر جاهزة');
        ...
        // دالة تكرار عرض السعر
        function duplicateQuote() {
            Swal.fire({
                title: 'تكرار عرض السعر',
                text: 'هل تريد إنشاء نسخة جديدة من عرض السعر الحالي؟',
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    // يمكن إضافة كود AJAX هنا لتكرار عرض السعر
                    Swal.fire({
                        title: 'تم!',
                        text: 'تم تكرار عرض السعر بنجاح',
                        icon: 'success',
        ...
        function confirmUpdate(event) {
            ...
            Swal.fire({
                title: 'تأكيد التحديث',
                text: 'هل أنت متأكد من تحديث عرض السعر؟',
                icon: 'question',
                showCancelButton: true,
