...
{
    return $this->hasMany(\App\Models\Visit::class, 'employee_id');
}
public function payments()
{
    return $this->hasMany(\App\Models\PaymentsProcess::class, 'employee_id')
                ->where('type', 'دفع'); // ✅ عدّل هذا النوع حسب جدولك
}

public function receipts()
{
    return $this->hasMany(\App\Models\PaymentsProcess::class, 'employee_id')
                ->where('type', 'سند'); // ✅ عدّل هذا النوع حسب جدولك
}

public function invoices()
{
    return $this->hasMany(\App\Models\Invoice::class, 'employee_id')
                ->where('is_paid', false); // ✅ فقط الفواتير غير المدفوعة
}
public function notes()
{
    return $this->hasMany(ClientRelation::class, 'employee_id');
}
